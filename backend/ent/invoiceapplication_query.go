...
type InvoiceApplicationQuery struct {
	...
	withUser   *UserQuery
	withOrders *InvoiceApplicationOrderQuery
...
func (_q *InvoiceApplicationQuery) QueryUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(invoiceapplication.Table, invoiceapplication.FieldID, selector),
...
func (_q *InvoiceApplicationQuery) QueryOrders() *InvoiceApplicationOrderQuery {
...
func (_q *InvoiceApplicationQuery) WithUser(opts ...func(*UserQuery)) *InvoiceApplicationQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *InvoiceApplicationQuery) WithOrders(opts ...func(*InvoiceApplicationOrderQuery)) *InvoiceApplicationQuery {
	query := (&InvoiceApplicationOrderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(invoiceapplication.FieldUserID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.InvoiceApplication.Query().
//		Select(invoiceapplication.FieldUserID).
//		Scan(ctx, &v)
...
func (_q *InvoiceApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*InvoiceApplication, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *InvoiceApplication, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withOrders; query != nil {
		if err := _q.loadOrders(ctx, query, nodes,
			func(n *InvoiceApplication) { n.Edges.Orders = []*InvoiceApplicationOrder{} },
			func(n *InvoiceApplication, e *InvoiceApplicationOrder) { n.Edges.Orders = append(n.Edges.Orders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *InvoiceApplicationQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*InvoiceApplication, init func(*InvoiceApplication), assign func(*InvoiceApplication, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*InvoiceApplication)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (_q *InvoiceApplicationQuery) loadOrders(ctx context.Context, query *InvoiceApplicationOrderQuery, nodes []*InvoiceApplication, init func(*InvoiceApplication), assign func(*InvoiceApplication, *InvoiceApplicationOrder)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(invoiceapplicationorder.FieldInvoiceApplicationID)
	}
	query.Where(predicate.InvoiceApplicationOrder(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(invoiceapplication.OrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.InvoiceApplicationID
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "invoice_application_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *InvoiceApplicationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUser != nil {
			_spec.Node.AddColumnOnce(invoiceapplication.FieldUserID)
		}
	}
