...
type PaymentOrderQuery struct {
	...
	withUser                    *UserQuery
	withInvoiceApplicationOrder *InvoiceApplicationOrderQuery
...
// QueryInvoiceApplicationOrder chains the current query on the "invoice_application_order" edge.
func (_q *PaymentOrderQuery) QueryInvoiceApplicationOrder() *InvoiceApplicationOrderQuery {
...
func (_q *PaymentOrderQuery) Clone() *PaymentOrderQuery {
	...
	return &PaymentOrderQuery{
		...
		withInvoiceApplicationOrder: _q.withInvoiceApplicationOrder.Clone(),
...
// WithInvoiceApplicationOrder tells the query-builder to eager-load the nodes that are connected to
// the "invoice_application_order" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PaymentOrderQuery) WithInvoiceApplicationOrder(opts ...func(*InvoiceApplicationOrderQuery)) *PaymentOrderQuery {
	query := (&InvoiceApplicationOrderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withInvoiceApplicationOrder = query
	return _q
}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	var (
		...
		loadedTypes = [2]bool{
			_q.withUser != nil,
			_q.withInvoiceApplicationOrder != nil,
		}
	)
	...
	if query := _q.withInvoiceApplicationOrder; query != nil {
		if err := _q.loadInvoiceApplicationOrder(ctx, query, nodes,
			func(n *PaymentOrder) { n.Edges.InvoiceApplicationOrder = []*InvoiceApplicationOrder{} },
			func(n *PaymentOrder, e *InvoiceApplicationOrder) {
				n.Edges.InvoiceApplicationOrder = append(n.Edges.InvoiceApplicationOrder, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *PaymentOrderQuery) loadInvoiceApplicationOrder(ctx context.Context, query *InvoiceApplicationOrderQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *InvoiceApplicationOrder)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(invoiceapplicationorder.FieldPaymentOrderID)
	}
	query.Where(predicate.InvoiceApplicationOrder(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.InvoiceApplicationOrderColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
