...
type SubscriptionPlan struct {
	...
	// 附赠订阅分组 ID 列表；有效期跟随主订阅
	BonusGroupIds []int64 `json:"bonus_group_ids,omitempty"`
...
func (*SubscriptionPlan) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case subscriptionplan.FieldBonusGroupIds:
			values[i] = new([]byte)
	...
	for i := range columns {
		...
		case subscriptionplan.FieldBonusGroupIds:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field bonus_group_ids", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.BonusGroupIds); err != nil {
					return fmt.Errorf("unmarshal field bonus_group_ids: %w", err)
				}
			}
...
func (_m *SubscriptionPlan) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("bonus_group_ids=")
	builder.WriteString(fmt.Sprintf("%v", _m.BonusGroupIds))
	builder.WriteString(", ")
