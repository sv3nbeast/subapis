...
// ImageStorageConfig configures the S3-compatible store used by async image jobs.
...
type GatewayConfig struct {
	...
	// OpenAIKiroBridgeEnabled: 允许 OpenAI 分组为指定文本模型调度显式授权的 Kiro OAuth 账号。
	// 默认关闭，作为生产灰度和紧急回滚开关。
	OpenAIKiroBridgeEnabled bool `mapstructure:"openai_kiro_bridge_enabled"`
...
// GatewayCNProvidersConfig configures balance probes for pay-as-you-go
// Kimi, Zhipu, and DeepSeek accounts. Coding-plan accounts use their rolling
// usage windows instead and are not governed by these thresholds.
...
// GatewayLiveConfig controls ChatGPT Frameless Live session limits.
type GatewayLiveConfig struct {
	// MaxSessionDurationSeconds is the hard upper bound for a Live session.
...
type GatewayOpenAIWSConfig struct {
	...
	// ClientFirstMessageTimeoutSeconds bounds reading and decompressing the first client message.
	ClientFirstMessageTimeoutSeconds int `mapstructure:"client_first_message_timeout_seconds"`
	// IngressInterTurnIdleTimeoutSeconds bounds client idle time between completed turns. Zero disables it.
	IngressInterTurnIdleTimeoutSeconds int `mapstructure:"ingress_inter_turn_idle_timeout_seconds"`
	// MaxIngressConnectionsPerAPIKey bounds live ingress sessions per API key. Zero disables it.
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	trustedProxiesConfigured := viper.InConfig("server.trusted_proxies") || trustedProxiesEnvConfigured
	...
	if forwardedHeadersEnvConfigured {
		cfg.Security.ForwardedClientIPHeaders = normalizeStringSlice(strings.Split(forwardedHeadersEnv, ","))
	}
...
func (c *Config) Validate() error {
	...
	if c.Gateway.KiroStreamKeepaliveInterval != 0 &&
		(c.Gateway.KiroStreamKeepaliveInterval < 5 || c.Gateway.KiroStreamKeepaliveInterval > 60) {
		return fmt.Errorf("gateway.kiro_stream_keepalive_interval must be 0 or between 5-60 seconds")
	}
