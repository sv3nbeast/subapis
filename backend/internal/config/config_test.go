...
func TestLoadDefaultGatewayTextMaxBodySize(t *testing.T) {
	resetViperWithJWTSecret(t)

	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, int64(32*1024*1024), cfg.Gateway.TextMaxBodySize)
	require.LessOrEqual(t, cfg.Gateway.TextMaxBodySize, cfg.Gateway.MaxBodySize)
}
...
func TestLoadDefaultOpenAIKiroBridgeDisabled(t *testing.T) {
	resetViperWithJWTSecret(t)

	cfg, err := Load()
	require.NoError(t, err)
	require.False(t, cfg.Gateway.OpenAIKiroBridgeEnabled)
}

func TestLoadOpenAIKiroBridgeEnabledFromEnv(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("GATEWAY_OPENAI_KIRO_BRIDGE_ENABLED", "true")

	cfg, err := Load()
	require.NoError(t, err)
	require.True(t, cfg.Gateway.OpenAIKiroBridgeEnabled)
}
...
func TestLoad_DefaultGatewayImageStreamConfig(t *testing.T) {
	...
	if cfg.Gateway.StreamDataIntervalTimeout != 900 {
		t.Fatalf("stream_data_interval_timeout = %d, want 900", cfg.Gateway.StreamDataIntervalTimeout)
	}
	...
	if cfg.Gateway.ImageStreamDataIntervalTimeout < cfg.Gateway.StreamDataIntervalTimeout {
		t.Fatalf("image stream timeout = %d, want at least ordinary stream timeout %d", cfg.Gateway.ImageStreamDataIntervalTimeout, cfg.Gateway.StreamDataIntervalTimeout)
	}
}
