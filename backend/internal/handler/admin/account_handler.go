...
func (h *AccountHandler) List(c *gin.Context) {
	...
	accounts, total, err := h.adminService.ListAccounts(c.Request.Context(), page, pageSize, platform, accountType, status, search, model, groupID, privacyMode, sortBy, sortOrder)
	if err != nil {
...
// Duplicate creates an independent account from an existing account's configuration.
...
func buildMappedKiroModelsFromDefaults(mapping map[string]string, defaultModels []kiropkg.Model) []kiropkg.Model {
	models := make([]kiropkg.Model, 0, len(mapping)+3)
	requestedModels := make([]string, 0, len(mapping))
	for requestedModel := range mapping {
		requestedModels = append(requestedModels, requestedModel)
	}
	...
	for _, nativeModelID := range []string{"gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"} {
		if _, explicitlyMapped := mapping[nativeModelID]; explicitlyMapped {
			continue
		}
		for _, defaultModel := range defaultModels {
			if defaultModel.ID == nativeModelID {
				models = append(models, defaultModel)
				break
			}
		}
