...
type channelModelPricingRequest struct {
	...
	CacheWrite5mPrice            *float64                   `json:"cache_write_5m_price" binding:"omitempty,min=0"`
...
func accountStatsRuleToResponse(rule *service.AccountStatsPricingRule) accountStatsPricingRuleResponse {
	resp := accountStatsPricingRuleResponse{
		ID:         rule.ID,
		Name:       rule.Name,
	...
	if resp.AccountIDs == nil {
		resp.AccountIDs = []int64{}
	}
	for _, pricing := range rule.Pricing {
		resp.Pricing = append(resp.Pricing, pricingToResponse(&pricing))
	}
	return resp
}
...
func pricingToResponse(p *service.ChannelModelPricing) channelModelPricingResponse {
	...
	return channelModelPricingResponse{
		...
		CacheWrite5mPrice:            p.CacheWrite5mPrice,
...
func pricingRequestToService(reqs []channelModelPricingRequest, allowChannelMultipliers bool) []service.ChannelModelPricing {
	...
	for _, r := range reqs {
		...
		result = append(result, service.ChannelModelPricing{
			Platform:                     platform,
			Models:                       r.Models,
			Disabled:                     r.Enabled != nil && !*r.Enabled,
			BillingMode:                  billingMode,
			InputPrice:                   r.InputPrice,
			OutputPrice:                  r.OutputPrice,
			CacheWritePrice:              r.CacheWritePrice,
			CacheWrite5mPrice:            r.CacheWrite5mPrice,
...
func accountStatsRulesRequestToService(reqs []accountStatsPricingRuleRequest) []service.AccountStatsPricingRule {
	result := make([]service.AccountStatsPricingRule, 0, len(reqs))
	for i, r := range reqs {
		sortOrder := r.SortOrder
		if sortOrder == 0 {
			sortOrder = i
		}
		result = append(result, service.AccountStatsPricingRule{
			Name:       r.Name,
			GroupIDs:   r.GroupIDs,
...
func timePricingRequestToService(value *channelTimePricingRequest) *service.ChannelTimePricing {
	...
	return &service.ChannelTimePricing{Timezone: value.Timezone, WeekdaysOnly: value.WeekdaysOnly, Periods: periods}
}
...
func timePricingToResponse(value *service.ChannelTimePricing) *channelTimePricingResponse {
	...
	return &channelTimePricingResponse{Timezone: value.Timezone, WeekdaysOnly: value.WeekdaysOnly, Periods: periods}
}
...
func (h *ChannelHandler) SyncPricingModels(c *gin.Context) {
	...
	models, ok := h.pricingService.ListChannelPricingModelNamesForPlatform(platform)
	if !ok {
