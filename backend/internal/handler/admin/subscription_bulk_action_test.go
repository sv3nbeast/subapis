...
// 本地 ExtendSubscription 在行锁事务闭包内同步配额周期（未过期 SetQuotaCycle /
// 已过期 ResetUsageForQuotaCycle，0b2a61d92 与 bf1e0bc3a）；官方 3d6c20772 的桩只
// 实现了 ExtendExpiry/UpdateStatus，内嵌的 nil 接口会在这两个调用上 panic。
// 两者都是同一次 extend 的附属写入，不计入 extendCalls。
func (r *bulkActionHandlerSubscriptionRepo) SetQuotaCycle(_ context.Context, _ int64, startAt, endAt time.Time, cycleDays int) error {
	r.sub.QuotaCycleStartAt, r.sub.QuotaCycleEndAt, r.sub.QuotaCycleDays = &startAt, &endAt, cycleDays
	return nil
}

func (r *bulkActionHandlerSubscriptionRepo) ResetUsageForQuotaCycle(_ context.Context, _ int64, windowStart, cycleStartAt, cycleEndAt time.Time, cycleDays int) error {
	r.sub.DailyUsageUSD, r.sub.WeeklyUsageUSD, r.sub.MonthlyUsageUSD = 0, 0, 0
	r.sub.DailyWindowStart, r.sub.WeeklyWindowStart, r.sub.MonthlyWindowStart = &windowStart, &windowStart, &windowStart
	r.sub.QuotaCycleStartAt, r.sub.QuotaCycleEndAt, r.sub.QuotaCycleDays = &cycleStartAt, &cycleEndAt, cycleDays
	return nil
}
