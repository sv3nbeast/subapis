...
func GroupFromServiceAdmin(g *service.Group) *AdminGroup {
	...
	out := &AdminGroup{
		...
		ModelPricing:                groupModelPricingFromService(g.ModelPricing),
...
func AccountFromServiceShallow(a *service.Account) *Account {
	...
	if a.IsAnthropicOAuthOrSetupToken() {
		if a.IsAnthropicStableIdentityEnabled() {
			out.AnthropicStableIdentityEnabled = true
			out.AnthropicStableIdentityState = a.AnthropicStableIdentityState()
			out.AnthropicStableIdentityBlocked = a.IsAnthropicStableIdentityBlocked()
		}
...
func redactAccountManagedExtra(extra map[string]any) map[string]any {
	...
	for key, value := range extra {
		if service.IsAnthropicStableIdentityManagedExtraKey(key) {
			continue
		}
		switch key {
...
func userSubscriptionFromServiceBase(sub *service.UserSubscription) UserSubscription {
	return UserSubscription{
		...
		QuotaCycleStartAt:  sub.QuotaCycleStartAt,
		QuotaCycleEndAt:    sub.QuotaCycleEndAt,
		QuotaCycleDays:     sub.QuotaCycleDays,
