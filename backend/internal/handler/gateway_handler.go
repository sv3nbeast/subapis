...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		for {
			...
			accountReleaseFunc := selection.ReleaseFunc
			// Kiro 粘性迁移延后：nianzs 引擎成功后再迁移绑定，等待路径据此跳过准入后绑定。
			deferKiroMigration := false
			if !selection.Acquired {
				if selection.WaitPlan == nil {
					reqLog.Warn("gateway.select_account_no_slot_no_wait_plan",
						zap.Int64("account_id", account.ID),
			...
			// Kiro 粘性迁移延后（nianzs 成功后再迁移）时跳过准入后绑定。
			if (selection.ProfitGateActive() || !selection.Acquired) && !deferKiroMigration {
...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	case service.PlatformDroid:
		writeModelsList(c, platform, droid.DefaultModelIDs())
		return
...
func modelListingSource(platform string, availableModels, fallbackModels []string) []string {
	...
	if platform == service.PlatformAnthropic {
		return mergeGatewayModelIDs(availableModels, fallbackModels)
	}
	return availableModels
}
...
func (h *GatewayHandler) AntigravityModels(c *gin.Context) {
	...
	allowlistEnabled := apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled()
...
func (h *GatewayHandler) handleStreamingAwareErrorWithCode(c *gin.Context, status int, errType, code, message string, streamStarted bool) {
	...
	if streamStarted {
		...
		if ok {
			...
			errorEvent := `data: {"type":"error","error":{"type":` + strconv.Quote(errType) + errorCode + `,"message":` + strconv.Quote(clientMessage) + `}}` + "\n\n"
	...
	h.errorResponseWithCode(c, status, errType, code, clientMessage)
}
...
func detectInterceptType(body []byte, model string, maxTokens int, isStream bool, isClaudeCodeClient bool) InterceptType {
	// 优先检查连通性探测请求（max_tokens=1, 非流式，任意模型，任意客户端）。
	// 涵盖 claude-cli、Claude Code Desktop Test Connection、Anthropic SDK 等所有客户端的探测。
	// 返回 mock 响应，零账号配额消耗。
	if isClaudeCodeConnectionProbeRequest(maxTokens, isStream) {
...
func billingErrorDetails(err error) (status int, code, message string, retryAfter int) {
	...
	if errors.Is(err, service.ErrSubscriptionModelQuotaExhausted) {
		msg := pkgerrors.Message(err)
		return http.StatusTooManyRequests, "rate_limit_exceeded", msg, extractQuotaResetSeconds(err)
	}
