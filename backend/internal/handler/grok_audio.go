...
func (h *OpenAIGatewayHandler) GrokRealtime(c *gin.Context) {
	...
	for attempts := 0; attempts < 4; attempts++ {
		...
		var slotErr error
		release, slotStatus, slotErr = h.acquireResponsesAccountSlot(c, apiKey.GroupID, "", candidate, false, &streamStarted, reqLog)
		if slotErr != nil {
			reqLog.Warn("grok_realtime.account_slot_failed", zap.Int64("account_id", account.ID), zap.Error(slotErr))
...
func (h *OpenAIGatewayHandler) GrokVoice(c *gin.Context, endpoint string) {
	...
	for attempts := 0; attempts < 4; attempts++ {
		...
		var started bool
		release, status, slotErr := h.acquireResponsesAccountSlot(c, apiKey.GroupID, "", selection, false, &started, reqLog)
		if slotErr != nil {
			failed[account.ID] = struct{}{}
			continue
		}
