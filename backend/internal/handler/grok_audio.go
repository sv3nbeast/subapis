...
func (h *OpenAIGatewayHandler) GrokRealtime(c *gin.Context) {
	...
	release, acquired, _ := h.acquireResponsesAccountSlot(c, apiKey.GroupID, "", selection, true, &streamStarted, reqLog)
	if !acquired {
		return
	}
	defer release()
...
func (h *OpenAIGatewayHandler) GrokVoice(c *gin.Context, endpoint string) {
	...
	for attempts := 0; attempts < 4; attempts++ {
		...
		var started bool
		release, acquired, _ := h.acquireResponsesAccountSlot(c, apiKey.GroupID, "", selection, false, &started, reqLog)
		if !acquired {
