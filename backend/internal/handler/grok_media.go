...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		if endpoint.IsGenerationRequest() && !endpoint.IsSeedance() {
			...
			if !eligible {
				...
				reqLog.Warn("grok_media.account_eligibility_rejected",
					zap.Int64("account_id", account.ID), zap.String("reason", eligibilityReason),
		...
		var accountAcquired openAISlotAcquireResult
		var slotErr error
		accountReleaseFunc, accountAcquired, slotErr = h.acquireResponsesAccountSlot(c, apiKey.GroupID, admissionSessionHash, selection, false, &streamStarted, reqLog)
		if slotErr != nil {
			h.handleConcurrencyError(c, slotErr, "account", streamStarted)
			return
		}
		if accountAcquired == openAISlotAcquireProfitVetoed {
			failedAccountIDs[account.ID] = struct{}{}
			continue
		}
