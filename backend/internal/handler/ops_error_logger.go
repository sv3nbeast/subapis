...
func normalizeOpsErrorType(errType string, code string) string {
	...
	// SLA classification. Structured provider errors with an explicit code keep
	// their provider type, while local business-limit codes remain authoritative.
	if errType == "model_not_found" {
		if mapped, ok := mapOpsErrorCodeToType(code); ok {
			return mapped
		}
	...
	if errType != "" && isKnownOpsErrorType(errType) && errType != "api_error" {
		return errType
	}
	if mapped, ok := mapOpsErrorCodeToType(code); ok {
		return mapped
