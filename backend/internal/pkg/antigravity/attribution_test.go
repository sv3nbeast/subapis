	...
	for _, tt := range tests {
		for _, arrayForm := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				var texts []string
				// 本地 buildSystemInstruction 对齐 agent-vibes：不注入 SYSTEM_PROMPT_END 结束标记，
				// 纯 attribution 元数据剥离后没有任何 part 时 systemInstruction 直接省略（nil）。
...
func TestTransformClaudeToGemini_AttributionDoesNotHideIdentity(t *testing.T) {
	...
	require.NoError(t, err)
	got := buildSystemInstruction(system, "gemini-3.8-flash-high", DefaultTransformOptions(), nil, false)
