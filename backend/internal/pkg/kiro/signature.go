...
func thinkingSignature(content string) string {
	return thinkingSignatureForMessage(content, "claude", "")
}

func thinkingSignatureForMessage(content, model, messageID string) string {
	if content == "" {
		return ""
	}

	cacheKey := hashSignatureCacheKey(content + ":" + model + ":" + messageID)
	...
	sig := generateClaudeThinkingSignature(content, model, messageID)
...
func generateClaudeThinkingSignature(thinkingContent, model, messageID string) string {
	nonce := make([]byte, 12)
...
func hashSignatureCacheKey(s string) string {
