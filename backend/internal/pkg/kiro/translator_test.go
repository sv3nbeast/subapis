...
func TestBuildKiroPayloadInlineSystemDoesNotChangeStableConversationID(t *testing.T) {
	withoutReminder := []byte(`{
		"model":"claude-sonnet-4-5",
	...
	first, err := BuildKiroPayloadWithContext(withoutReminder, "claude-sonnet-4.5", "", "AI_EDITOR", nil)
	require.NoError(t, err)
	second, err := BuildKiroPayloadWithContext(withReminder, "claude-sonnet-4.5", "", "AI_EDITOR", nil)
	require.NoError(t, err)
	...
	require.NotContains(t,
		gjson.GetBytes(second.Payload, "conversationState.history.0.userInputMessage.content").String(),
		"late reminder",
	)
}
