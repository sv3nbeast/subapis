...
func TestBuildKiroPayloadSetsContextWindowFallbackForCompactionUsage(t *testing.T) {
	body := []byte(`{"model":"claude-sonnet-5","messages":[{"role":"user","content":"hello"}]}`)
	result, err := BuildKiroPayloadWithContext(body, "claude-sonnet-5", "", "AI_EDITOR", nil)
	require.NoError(t, err)
	require.Equal(t, 1_000_000, result.Context.ContextWindowTokens)
	require.Equal(t, "static_fallback", result.Context.ContextWindowSource)

	legacyBody := []byte(`{"model":"claude-sonnet-4-5","messages":[{"role":"user","content":"hello"}]}`)
	legacyResult, err := BuildKiroPayloadWithContext(legacyBody, "claude-sonnet-4.5", "", "AI_EDITOR", nil)
	require.NoError(t, err)
	require.Equal(t, 200_000, legacyResult.Context.ContextWindowTokens)
}

func TestKiroContextUsageAt85PercentReturnsCompactionThresholdNonStreaming(t *testing.T) {
	stream := bytes.NewBuffer(nil)
	_, _ = stream.Write(buildEventStreamFrame(t, "assistantResponseEvent", map[string]any{
	...
	_, _ = stream.Write(buildEventStreamFrame(t, "contextUsageEvent", map[string]any{
		"contextUsagePercentage": 85.0,
	}))
	_, _ = stream.Write(buildEventStreamFrame(t, "messageMetadataEvent", map[string]any{
		"messageMetadataEvent": map[string]any{
			"tokenUsage": map[string]any{"outputTokens": 10, "totalTokens": 10},
		},
	}))
	...
	result, err := ParseNonStreamingEventStreamWithContext(stream, "claude-sonnet-5", KiroRequestContext{
		...
		CacheEmulationUsage: &Usage{
			...
			CacheCreation1hInputTokens: 7_000,
		},
	})
	require.NoError(t, err)
...
func TestKiroContextUsageBoundaryStaysBelow85UntilProviderReachesIt(t *testing.T) {
	usage := Usage{
		...
		ContextUsagePercentage:   84.999,
	}
	below := reconcileKiroUsageWithContext(usage, 1_000_000)
	require.Equal(t, 849_990, below.TotalTokens)
	require.Less(t, below.TotalTokens, 850_000)

	usage.ContextUsagePercentage = 85
	atThreshold := reconcileKiroUsageWithContext(usage, 1_000_000)
	require.Equal(t, 850_000, atThreshold.TotalTokens)
}

func TestKiroStreamingContextUsageReturnsAuthoritativeTerminalUsage(t *testing.T) {
	stream := bytes.NewBuffer(nil)
	_, _ = stream.Write(buildEventStreamFrame(t, "assistantResponseEvent", map[string]any{
		"assistantResponseEvent": map[string]any{"content": "done"},
	}))
	_, _ = stream.Write(buildEventStreamFrame(t, "contextUsageEvent", map[string]any{
		"contextUsageEvent": map[string]any{"contextUsagePercentage": 85.58370208740234},
	}))
	_, _ = stream.Write(buildEventStreamFrame(t, "messageMetadataEvent", map[string]any{
		"messageMetadataEvent": map[string]any{
	...
	var out bytes.Buffer
	result, err := StreamEventStreamAsAnthropicWithContext(context.Background(), stream, &out, "claude-sonnet-5", 100_000, KiroRequestContext{
		ContextWindowTokens: 1_000_000,
	})
	require.NoError(t, err)
	require.False(t, result.Usage.InputTokensFromContext)
...
func TestKiroExactUpstreamInputTakesPriorityOverContextPercentage(t *testing.T) {
	stream := bytes.NewBuffer(nil)
	_, _ = stream.Write(buildEventStreamFrame(t, "assistantResponseEvent", map[string]any{
		"assistantResponseEvent": map[string]any{"content": "done"},
	}))
	...
	result, err := ParseNonStreamingEventStreamWithContext(stream, "claude-sonnet-5", KiroRequestContext{ContextWindowTokens: 1_000_000})
	require.NoError(t, err)
