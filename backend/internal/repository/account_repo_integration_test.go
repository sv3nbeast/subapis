...
func (s *AccountRepoSuite) TestListWithFilters() {
	tests := []struct {
		...
		{
			name: "filter_by_status_active_and_model_respects_antigravity_model_alias",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &service.Account{
					Name:     "opus-available",
					Status:   service.StatusActive,
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			accounts, page, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, tt.platform, tt.accType, tt.status, tt.search, tt.model, tt.groupID, tt.privacyMode)
			s.Require().NoError(err)
