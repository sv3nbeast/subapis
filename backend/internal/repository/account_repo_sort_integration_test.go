...
func (s *AccountRepoSuite) TestListWithFilters_ModelFilterPreservesSort() {
	mustCreateAccount(s.T(), s.client, &service.Account{Name: "a-model-account", Platform: service.PlatformAntigravity, Status: service.StatusActive})
	mustCreateAccount(s.T(), s.client, &service.Account{Name: "z-model-account", Platform: service.PlatformAntigravity, Status: service.StatusActive})
	...
	accounts, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{
		...
		SortBy:    "name",
		SortOrder: "desc",
	}, service.PlatformAntigravity, "", service.StatusActive, "", "claude-sonnet-4-6", 0, "")
	s.Require().NoError(err)
	s.Require().Len(accounts, 2)
	s.Require().Equal("z-model-account", accounts[0].Name)
	s.Require().Equal("a-model-account", accounts[1].Name)
}
