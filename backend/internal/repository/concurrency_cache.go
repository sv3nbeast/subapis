...
const (
	...
	countTokensUserActiveIndexKey = "concurrency:count_tokens:user:active_index"
...
var (
	...
	countTokensSlotIndex = slotIndexSpec{indexKey: countTokensUserActiveIndexKey, slotKey: countTokensUserSlotKey}
)
...
// Count-token admission never consumes regular user/live slots.
func (c *concurrencyCache) AcquireCountTokensUserSlot(ctx context.Context, userID int64, maxConcurrency int, requestID string) (bool, error) {
	result, now, err := runScriptInt64Pair(ctx, c.rdb, acquireScript, []string{countTokensUserSlotKey(userID)}, maxConcurrency, c.slotTTLSeconds, requestID)
	if err != nil {
		return false, err
	}
	if result == 1 {
		c.touchActiveIndexAt(ctx, countTokensUserActiveIndexKey, userID, now+int64(c.slotTTLSeconds))
	}
	return result == 1, nil
}
func (c *concurrencyCache) ReleaseCountTokensUserSlot(ctx context.Context, userID int64, requestID string) error {
	...
	c.refreshActiveIndex(ctx, countTokensUserActiveIndexKey, userID, key, "")
	return nil
}
...
func (c *concurrencyCache) CleanupExpiredAccountSlotKeys(ctx context.Context) error {
	for _, spec := range []slotIndexSpec{accountSlotIndex, userSlotIndex, countTokensSlotIndex} {
		if err := c.reconcileExpiredIndexCandidates(ctx, spec); err != nil {
			return err
		}
	}
	return nil
...
func (c *concurrencyCache) CleanupStaleProcessSlots(ctx context.Context, activeRequestPrefix string) error {
	...
	for _, spec := range []slotIndexSpec{accountSlotIndex, userSlotIndex, countTokensSlotIndex} {
		...
		if err := c.cleanupStaleProcessSlotsForIndex(ctx, spec, members, activeRequestPrefix, now); err != nil {
			return err
		}
	}
	return nil
