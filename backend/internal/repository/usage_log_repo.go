...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	imageSizeBreakdown := usageLogImageSizeBreakdownArg(log.ImageSizeBreakdown)
...
func (r *usageLogRepository) GetDailyStatsAggregated(ctx context.Context, userID int64, startTime, endTime time.Time) (result []map[string]any, err error) {
	...
	for rows.Next() {
		...
		result = append(result, map[string]any{
			...
			"total_cache_tokens":          totalCacheCreationTokens + totalCacheReadTokens,
			"total_tokens":                totalInputTokens + totalOutputTokens + totalCacheCreationTokens + totalCacheReadTokens,
...
// GetAllGroupUsageSummary returns today/yesterday and retained cumulative costs
// using the configured timezone and the shared rollup/tail boundary.
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	if imageSizeBreakdown.Valid && strings.TrimSpace(imageSizeBreakdown.String) != "" {
...
func usageLogImageSizeBreakdownArg(v map[string]int) any {
	if len(v) == 0 {
		return sql.NullString{}
	}
