...
// kiro_credits/session_id 之前，与参数类型表保持同位；缺失时落 NULL 而不是空串。
// 本地列序：… account_stats_cost, upstream_request_id, kiro_credits, session_id,
// native_compaction_v2, created_at（官方无 kiro_credits，故索引为倒数第 5 而非第 4）。
func TestPrepareUsageLogInsert_UpstreamRequestIDArgWiring(t *testing.T) {
	...
	require.Equal(t, "upstream_request_id", usageLogInsertColumns[idx])
	require.Equal(t, "kiro_credits", usageLogInsertColumns[idx+1])
	require.Equal(t, "session_id", usageLogInsertColumns[idx+2])
