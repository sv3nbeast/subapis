...
func TestGatewayRoutesGroupModelAllowlistMountedOnEveryGatewayRoute(t *testing.T) {
	...
	// 本地链：requireGroupAnthropic 先行（未分组 Key 直接以 Anthropic 格式拒绝），
	// 末位追加 claudeCodeOnlyEndpoints（Grok 分组仅放行 CLI 入口）；白名单位置不变。
	rootHelper := regexp.MustCompile(regexp.QuoteMeta(`r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), requireGroupAnthropic, groupModelAllowlist, compositeTarget, claudeCodeOnlyEndpoints, handler)`))
	...
	codexDirect := regexp.MustCompile(regexp.QuoteMeta(`codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), requireGroupAnthropic, groupModelAllowlist, compositeTarget, claudeCodeOnlyEndpoints)`))
