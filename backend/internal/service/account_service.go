...
type AccountRepository interface {
	...
	ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search, model string, groupID int64, privacyMode string) ([]Account, *pagination.PaginationResult, error)
...
func (s *AccountService) Create(ctx context.Context, req CreateAccountRequest) (*Account, error) {
	...
	if account.Type == AccountTypeAPIKey && len(req.GroupIDs) > 0 {
		for _, gid := range req.GroupIDs {
			...
			if g.RequireOAuthOnly && (g.Platform == PlatformOpenAI || g.Platform == PlatformAntigravity || g.Platform == PlatformAnthropic || g.Platform == PlatformGemini || g.Platform == PlatformDroid || g.Platform == PlatformGrok || g.Platform == PlatformCursor) {
...
func (s *AccountService) Update(ctx context.Context, id int64, req UpdateAccountRequest) (*Account, error) {
	...
	if req.Extra != nil {
		...
		extra = preserveAnthropicStableIdentityManagedExtra(account.Extra, extra)
...
func (s *AccountService) TestCredentials(ctx context.Context, id int64) error {
	...
	case PlatformKimi, PlatformZhipu, PlatformDeepseek:
		...
		return nil
	case PlatformCursor:
		// Cursor：凭证经 AgentService/Run 路径与账号测试验证。
		return nil
	default:
