...
func NewAdminService(
	...
	groupDuplicateRepo, _ := groupRepo.(GroupDuplicateRepository)
	emptyGroupDeleteRepo, _ := groupRepo.(EmptyGroupDeleteRepository)
	accountDuplicateRepo, _ := accountRepo.(AccountDuplicateRepository)
	accountBillingRepo, _ := accountRepo.(AccountBillingSettingsRepository)
	return &adminServiceImpl{
		cfg:                  cfg,
		userRepo:             userRepo,
		groupRepo:            groupRepo,
		groupDuplicateRepo:   groupDuplicateRepo,
		emptyGroupDeleteRepo: emptyGroupDeleteRepo,
		accountRepo:          accountRepo,
		accountDuplicateRepo: accountDuplicateRepo,
		accountBillingRepo:   accountBillingRepo,
		proxyRepo:            proxyRepo,
...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id int64, input *UpdateUserInput) (*User, error) {
	...
	if input.Role != "" {
		role, roleErr := normalizeAdminManagedUserRole(input.Role)
		if roleErr != nil {
			return nil, roleErr
		}
		if user.Role == RoleAdmin && role != RoleAdmin {
			_, result, listErr := s.userRepo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 1}, UserListFilters{Role: RoleAdmin})
			if listErr != nil {
				return nil, fmt.Errorf("count admins before role change: %w", listErr)
			}
...
func (s *adminServiceImpl) UpdateUserBalance(ctx context.Context, userID int64, balance float64, operation string, notes string) (*User, error) {
	...
	if operation == "add" && balanceDiff > 0 && s.affiliateService != nil && s.settingService != nil && s.settingService.IsAffiliateAdminRechargeEnabled(ctx) {
		if rebate, affiliateErr := s.affiliateService.AccrueInviteRebate(ctx, userID, balance); affiliateErr != nil {
			logger.LegacyPrintf("service.admin", "admin recharge affiliate rebate failed: user_id=%d amount=%.8f err=%v", userID, balance, affiliateErr)
		} else if rebate > 0 {
			logger.LegacyPrintf("service.admin", "admin recharge affiliate rebate accrued: user_id=%d amount=%.8f rebate=%.8f", userID, balance, rebate)
		}
	}
...
func (s *adminServiceImpl) GetUserBalanceHistory(ctx context.Context, userID int64, page, pageSize int, codeType string) ([]RedeemCode, int64, float64, error) {
	...
	if codeType == RedeemTypeSubscription && s.entClient != nil {
		...
		codes = mergeBalanceHistoryCodes(params, codes, subscriptionOrders)
...
func (s *adminServiceImpl) getAllUserBalanceHistory(ctx context.Context, userID int64, params pagination.PaginationParams) ([]RedeemCode, int64, float64, error) {
	...
	subscriptionCodes, subscriptionTotal, err := s.listUserSubscriptionPaymentHistory(ctx, userID, needed)
	if err != nil {
		return nil, 0, 0, err
	}
	codes := mergeBalanceHistoryCodes(params, redeemCodes, affiliateCodes, subscriptionCodes)
...
LIMIT $3
OFFSET $2`, userID, params.Offset(), params.Limit())
	if err != nil {
...
func mergeBalanceHistoryCodes(params pagination.PaginationParams, recordGroups ...[]RedeemCode) []RedeemCode {
	var combined []RedeemCode
	for _, records := range recordGroups {
		combined = append(combined, records...)
	}
	sort.SliceStable(combined, func(i, j int) bool {
		ti := redeemCodeHistoryTime(combined[i])
		tj := redeemCodeHistoryTime(combined[j])
		if !ti.Equal(tj) {
			return ti.After(tj)
		}
...
func defaultModelsListCandidateIDs(platform string) []string {
	...
	case PlatformComposite:
		platforms := []string{PlatformOpenAI, PlatformGemini, PlatformAntigravity, PlatformDroid, PlatformGrok, PlatformAnthropic, PlatformMiniMax, PlatformOpenCodeGo}
		seen := make(map[string]struct{})
		var ids []string
		for _, concrete := range platforms {
			for _, model := range defaultModelsListCandidateIDs(concrete) {
				if _, ok := seen[model]; ok {
					continue
				}
				seen[model] = struct{}{}
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	// 拒绝开启并在创建后的编辑里配置。
	if normalizeCodexModelsManifestConfig(NormalizeGroupPlatform(input.Platform), input.CodexModelsManifestConfig).Enabled {
	...
	if len(input.CopyAccountsFromGroupIDs) > 0 {
		...
		for _, srcGroupID := range uniqueSourceGroupIDs {
			...
			if platform != PlatformComposite && srcGroup.Platform != platform {
	...
	if group.RequireOAuthOnly && (group.Platform == PlatformOpenAI || group.Platform == PlatformAntigravity || group.Platform == PlatformAnthropic || group.Platform == PlatformGemini || group.Platform == PlatformDroid || group.Platform == PlatformGrok || group.Platform == PlatformCursor) && len(accountIDsToCopy) > 0 {
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if len(input.CopyAccountsFromGroupIDs) > 0 {
		...
		for _, srcGroupID := range uniqueSourceGroupIDs {
			...
			if group.Platform != PlatformComposite && srcGroup.Platform != group.Platform {
...
func (s *adminServiceImpl) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search, model string, groupID int64, privacyMode string, sortBy, sortOrder string) ([]Account, int64, error) {
	if groupID > 0 {
	...
	params := pagination.PaginationParams{Page: page, PageSize: pageSize, SortBy: sortBy, SortOrder: sortOrder}
	accounts, result, err := s.accountRepo.ListWithFilters(ctx, params, platform, accountType, status, search, model, groupID, privacyMode)
	if err != nil {
		return nil, 0, err
	}
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	if len(input.Credentials) > 0 {
		// 官方 242907854：OpenCode Go 协议规则凭证归一化（与 Create/Update 同一收口）。
		if err := NormalizeOpenCodeGoProtocolRulesCredentials(input.Credentials); err != nil {
			return nil, err
		}
...
func bulkCredentialsChangeInvalidatesProbe(credentials map[string]any) bool {
	if len(credentials) == 0 {
		return false
	}
