...
func TestAntigravityGatewayService_ForwardGemini_ClearsStickySessionOnGeminiRateLimit(t *testing.T) {
	// 官方新增测试，断言 antigravity Gemini 限流统一返回 503 failover + 清粘性会话。
	// 本地 antigravity 限流为 fork 私有实现（model-rate-limited 返回 429 切账号），与官方
	// 503 设计不同；该行为差异是上游同步待评估项，非本地功能回归。暂跳过，待后续决定是否
	// 采纳官方 503 统一逻辑。详见 docs/upstream-sync-conflict-ledger.md。
	t.Skip("官方 antigravity Gemini 503 sticky-clear 行为待评估采纳；本地为 429 fork 实现")
	gin.SetMode(gin.TestMode)
	writer := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(writer)
