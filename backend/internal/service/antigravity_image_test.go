...
func TestExtractImageSize_ValidSizes(t *testing.T) {
	...
	require.Equal(t, "1K", NormalizeImageBillingTierOrDefault(svc.extractImageSize(body)))

	// 2K
	body = []byte(`{"generationConfig":{"imageConfig":{"imageSize":"2K"}}}`)
	require.Equal(t, "2K", NormalizeImageBillingTierOrDefault(svc.extractImageSize(body)))

	// 4K
	body = []byte(`{"generationConfig":{"imageConfig":{"imageSize":"4K"}}}`)
	require.Equal(t, "4K", NormalizeImageBillingTierOrDefault(svc.extractImageSize(body)))
}
