...
const apiKeyAuthSnapshotVersion = 25 // v25: local v23 shared subscription model quota groups + official v24 group model_allowlist/codex_models_manifest_config (renamed from models_list_config)
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	if apiKey.Group != nil {
		...
		snapshot.Group = &APIKeyAuthGroupSnapshot{
			...
			KiroAnthropicFallbackEnabled:    groupForSnapshot.EffectiveKiroAnthropicFallbackEnabled(),
			KiroAnthropicFallbackFirstSemanticTimeoutSeconds: groupForSnapshot.EffectiveKiroAnthropicFallbackFirstSemanticTimeoutSeconds(),
			KiroAnthropicFallbackMaxAnthropicAttempts:        groupForSnapshot.EffectiveKiroAnthropicFallbackMaxAnthropicAttempts(),
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	if snapshot.Group != nil {
		apiKey.Group = &Group{
			...
			KiroAnthropicFallbackEnabled:    snapshot.Group.KiroAnthropicFallbackEnabled,
			KiroAnthropicFallbackFirstSemanticTimeoutSeconds: snapshot.Group.KiroAnthropicFallbackFirstSemanticTimeoutSeconds,
			KiroAnthropicFallbackMaxAnthropicAttempts:        snapshot.Group.KiroAnthropicFallbackMaxAnthropicAttempts,
