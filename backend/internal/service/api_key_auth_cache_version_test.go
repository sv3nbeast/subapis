...
func TestAPIKeyService_RejectsV18AuthSnapshotWithoutSubscriptionModelQuotaRatios(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-legacy-subscription-model-quota", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v18 auth snapshot to be rejected after subscription model quota ratios were added")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
...
func TestAPIKeyService_RejectsV19AuthSnapshotWithoutHydratedSubscriptionModelQuotaRatios(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-unhydrated-subscription-model-quota", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v19 auth snapshot to be rejected after model quota hydration was fixed")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
