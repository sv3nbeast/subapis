...
var (
	...
	ErrEmailDomainRegistrationLimit = infraerrors.BadRequest("EMAIL_DOMAIN_REGISTRATION_LIMIT", "this email domain cannot register another account; use a mainstream email or contact support to add the enterprise domain")
...
func buildEmailSuffixBlockedError(email string) error {
	blockedSuffix := RegistrationEmailSuffix(email)
	if blockedSuffix == "" {
		return ErrEmailSuffixBlocked
	}
	return infraerrors.BadRequest(
		"EMAIL_SUFFIX_BLOCKED",
		fmt.Sprintf("email suffix is blocked: %s", blockedSuffix),
	).WithMetadata(map[string]string{
		"blocked_suffix": blockedSuffix,
	})
}
