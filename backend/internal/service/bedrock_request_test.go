...
func TestResolveBedrockModelID(t *testing.T) {
	...
	t.Run("default opus 4.8 alias is enabled for Bedrock", func(t *testing.T) {
		account := &Account{
			Platform: PlatformAnthropic,
			Type:     AccountTypeBedrock,
			Credentials: map[string]any{
	...
	t.Run("default opus 4.8 thinking alias resolves to base Bedrock model", func(t *testing.T) {
		account := &Account{
			Platform: PlatformAnthropic,
			Type:     AccountTypeBedrock,
			Credentials: map[string]any{
		...
		modelID, ok := ResolveBedrockModelID(account, "claude-opus-4-8-thinking")
		require.True(t, ok)
		assert.Equal(t, "eu.anthropic.claude-opus-4-8-v1", modelID)
	})
