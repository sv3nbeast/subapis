...
func defaultMaxReasoningEffortMultiplier(model string) *float64 {
	if !applyDefaultMaxReasoningEffortMultiplier || !isClaudeFable51Model(model) {
		return nil
	}
...
func (s *BillingService) initFallbackPricing() {
	...
	opus5Pricing := &ModelPricing{
		InputPricePerToken:                 5e-6,    // $5 per MTok
		InputPricePerTokenPriority:         10e-6,   // $10 per MTok (Fast 2x)
		OutputPricePerToken:                25e-6,   // $25 per MTok
		OutputPricePerTokenPriority:        50e-6,   // $50 per MTok (Fast 2x)
		CacheCreationPricePerToken:         6.25e-6, // $6.25 per MTok
		CacheCreationPricePerTokenPriority: 12.5e-6, // $12.50 per MTok (Fast 2x)
		CacheReadPricePerToken:             0.5e-6,  // $0.50 per MTok
		CacheReadPricePerTokenPriority:     1e-6,    // $1 per MTok (Fast 2x)
	...
	// Antigravity thinking-tier aliases share the public Gemini 3.6 Flash rate.
	...
	// OpenAI GPT-5.5（官方 Codex token 价格为 GPT-5.4 的 2 倍）
	s.fallbackPrices["gpt-5.5"] = &ModelPricing{
		...
		CacheReadPricePerTokenPriority: 1.25e-6, // $1.25 per MTok (Fast 2.5x)
		SupportsCacheBreakdown:         false,
	...
	// OpenAI GPT-6 Sol / Luna 官方价格（2026-09-22 发布）。缓存写入为输入价的 1.25 倍，
	// >272K 输入时 input/cache ×2、output ×1.5，Fast ×2。
	...
	// Sol 为官方 2026-11-21 前的现行价 $4/$20（Fast 2x）；届时需按官方页复核。
...
func (s *BillingService) getModelPricingAt(model string, pricingAt time.Time) (*ModelPricing, error) {
	...
	if fallback != nil {
		...
		return s.applyModelSpecificPricingPolicyEx(model, cloneModelPricing(fallback), true, pricingAt), nil
	}
...
func (s *BillingService) calculatePerRequestCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	if unitPrice == 0 && resolved.Mode == BillingModeImage {
		unitPrice = imageTierFallbackPrice(input.Resolver, resolved, input.SizeTier)
	}
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool, pricingAt time.Time) *ModelPricing {
	...
	// GPT-5.6 与 GPT-6 全族的官方缓存写入价均为输入价的 1.25 倍（Sol $2→$2.5、
	// Luna $0.1→$0.125）。远程 LiteLLM 目录对新模型常缺 cache_creation 字段，
	// 漏掉这里会让缓存写入按 0 计费。
	isGPT56 := isOpenAIGPT56Model(normalized) || isOpenAIGPT6Model(normalized)
...
// 的标准价倍率：gpt-5.6 / gpt-6 全族 / gpt-5.4 为 2x，gpt-5.5 为 2.5x。未定义 Fast
...
// CalculateCostWithLongContext preserves the explicit legacy long-context
// contract used by the Gemini gateway. The internal calls disable catalog
// long-context policy so the explicit split is never multiplied twice.
func (s *BillingService) CalculateCostWithLongContext(model string, tokens UsageTokens, rateMultiplier float64, threshold int, extraMultiplier float64) (*CostBreakdown, error) {
	calculateBase := func(part UsageTokens, multiplier float64) (*CostBreakdown, error) {
		return s.calculateCostInternalWithPolicy(model, part, multiplier, "", nil, false)
	}
	if threshold <= 0 || extraMultiplier <= 1 {
		return calculateBase(tokens, rateMultiplier)
	}
	...
	inRangeCost, err := calculateBase(inRangeTokens, rateMultiplier)
	if err != nil {
		return nil, err
	}
	outRangeCost, err := calculateBase(UsageTokens{
		InputTokens: outRangeInputTokens, CacheReadTokens: outRangeCacheTokens,
...
func imageTierFallbackPrice(resolver *ModelPricingResolver, resolved *ResolvedPricing, requestedTier string) float64 {
	if resolver == nil || resolved == nil {
		return 0
	}
	switch requestedTier {
	case ImageBillingSize2K:
		for _, tier := range []string{ImageBillingSize1K, ImageBillingSize4K} {
			if price := resolver.GetRequestTierPrice(resolved, tier); price > 0 {
				return 0
			}
		}
	case ImageBillingSize1K, ImageBillingSize4K:
		return 0
	}
	return resolver.GetRequestTierPrice(resolved, ImageBillingSize2K)
}
