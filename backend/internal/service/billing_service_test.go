...
func TestGetModelPricing_OpenAIGPT55FallbackUsesDedicatedPricing(t *testing.T) {
	svc := newTestBillingService()
...
func TestGetModelPricing_OpenAIGPT55FallbackSupportsHyphenlessAlias(t *testing.T) {
	svc := newTestBillingService()

	pricing, err := svc.GetModelPricing("gpt5.5")
	require.NoError(t, err)
	require.NotNil(t, pricing)
	require.InDelta(t, 5e-6, pricing.InputPricePerToken, 1e-12)
	require.InDelta(t, 30e-6, pricing.OutputPricePerToken, 1e-12)
...
func TestGetModelPricing_OpenAICompactAliasesFallback(t *testing.T) {
	...
	tests := []struct {
		...
		{model: "openai/gpt5.5", inputPrice: 5e-6, outputPrice: 30e-6, cacheRead: 0.5e-6, longContext: 272000},
...
func TestGetFallbackPricing_FamilyMatching(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "openai hyphenless gpt5.5", model: "gpt5.5", expectedInput: 5e-6},
		...
		{
			// 官方现行 Flash ID（DeepSeek-V4.1-Flash，2026-09-10 起）。
			name:              "deepseek flash current id",
		...
		{
			// 已退役，官方由 V4.1-Flash 承接并按 Flash 新价计费。
...
func TestGetModelPricingWithChannel_CacheWriteBreakdownOverrides(t *testing.T) {
	svc := newTestBillingService()
...
func TestComputeCacheCreationCost_ChannelCacheWriteBreakdown(t *testing.T) {
	svc := newTestBillingService()

	pricing := &ModelPricing{
		CacheCreationPricePerToken: 7e-6,
		CacheCreation5mPrice:       4e-6,
		CacheCreation1hPrice:       9e-6,
		SupportsCacheBreakdown:     true,
	}
	tokens := UsageTokens{
	...
	require.InDelta(t, 0.0022, svc.computeCacheCreationCost(pricing, tokens, 1, 1), 1e-12)
}
