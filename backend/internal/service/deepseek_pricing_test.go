...
func TestCalculateCostUnified_DeepseekDefaultCardPeakMultiplier(t *testing.T) {
	...
	// 低谷成本：1000*1.5e-7 + 500*6e-7 + 1000*3e-9 = 4.53e-4
...
func TestGetModelPricing_DeepseekForcesOfficialRatesOverJSON(t *testing.T) {
	...
	tests := []struct {
		...
		// 已退役的 v4-flash / vision-exp：官方由 V4.1-Flash 承接并按 Flash 新价计费。
...
func TestGetModelPricing_UnknownDeepseekMapsToFlash(t *testing.T) {
	...
	for _, m := range []string{"deepseek-v3-2-251201", "deepseek-chat", "deepseek-reasoner", "deepseek-foo", "deepseek-v4.1-flash"} {
