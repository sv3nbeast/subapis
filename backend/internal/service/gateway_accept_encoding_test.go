	...
	for _, protocol := range []struct {
		...
		t.Run(protocol.name, func(t *testing.T) {
			...
			for _, tc := range []struct {
				...
				// 本地（8ad4e3bf5 对齐真实 Claude CLI 抓包）：客户端未带 Accept-Encoding 时
				// applyClaudeOAuthHeaderDefaults 补 CLI 默认值，而非交给 net/http 自动加 gzip；
				// 上游 transport DisableCompression=true + decompressResponseBody 自行解 br/zstd。
				// 官方本用例的关注点（同名 header 不重复、显式协商被 net/http 识别）仍由单值断言覆盖。
				{name: "omitted", want: claude.DefaultAcceptEncodingHeader},
			} {
