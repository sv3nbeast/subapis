...
func TestGatewayService_AnthropicAPIKeyPassthrough_ModelMappingEdgeCases(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "Forward: 无账号映射时应用 Anthropic 4.7 thinking 默认别名",
		...
		{
			name:          "CountTokens: 无账号映射时应用 Anthropic 4.7 thinking 默认别名",
			model:         "claude-opus-4.7-thinking",
			modelMapping:  nil,
			expectedModel: "claude-opus-4-7",
			endpoint:      "count_tokens",
		...
		{
			name:          "CountTokens: 无账号映射时应用 Anthropic 4.6 thinking 默认别名",
			model:         "claude-opus-4.6-thinking",
			modelMapping:  nil,
			expectedModel: "claude-opus-4-6",
			endpoint:      "count_tokens",
...
func TestGatewayService_AnthropicOAuth_ForwardPreservesBillingHeaderSystemBlock(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			arr := system.Array()
			require.Len(t, arr, 3, "system array should have billing block + Claude Code identity + expansion block")
			...
			require.Contains(t, arr[2].Get("text").String(), "You are an interactive agent")
...
func TestGatewayService_AnthropicOAuth_SystemPromptInjectionCanBeDisabled(t *testing.T) {
	...
	require.Regexp(t, `cch=[0-9a-f]{5};`, system.Get("0.text").String())
	require.Equal(t, "Original system prompt", system.Get("1.text").String())
...
func TestGatewayService_AnthropicOAuth_OfficialCompactionPreservesNativeNonStreamAndHelper(t *testing.T) {
	...
	require.Equal(t, "ephemeral", gjson.GetBytes(upstream.lastBody, "system.1.cache_control.type").String(), "cacheable prefix must remain intact after the billing block")
...
func TestGatewayService_AnthropicOAuth_OfficialAgentClassifierPreservesNativeNonStream(t *testing.T) {
	...
	require.True(t, strings.HasPrefix(gjson.GetBytes(upstream.lastBody, "system.1.text").String(), claudeCodeAgentClassifierSystemPrefix))
	require.Equal(t, "ephemeral", gjson.GetBytes(upstream.lastBody, "system.1.cache_control.type").String())
