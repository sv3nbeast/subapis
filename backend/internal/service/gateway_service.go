...
func (s *GatewayService) GenerateSessionHash(parsed *ParsedRequest) string {
	...
	// 2.5. Kiro 分组专用：使用对话生命周期内相对稳定的内容做 hash。
	// Kiro 常见客户端会逐轮重放上下文并生成新的 conversationId；完整 messages hash 会随轮次漂移。
...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	...
	dropChannelRestricted := func(list []*Account) []*Account {
		if !needsUpstreamCheck {
			return list
		}
		kept := make([]*Account, 0, len(list))
			...
			if len(routingAvailable) > 0 {
				...
				for _, item := range routingAvailable {
					...
					if err == nil && result.Acquired {
						...
						if s.shouldBindSelectionBeforeSuccess(ctx, item.account, groupID, sessionHash, preserveKiroStickyBinding, deferKiroStickyMigration) && s.cache != nil {
	...
	if len(routingAccountIDs) == 0 && sessionHash != "" && stickyAccountID > 0 && !isExcluded(stickyAccountID) {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if ok {
				...
				if !clearSticky && platformOK && modelSupported && channelOK && modelSchedulable && quotaOK && windowCostOK && rpmOK && schedulable && profitOK {
	...
	candidates := dropChannelRestricted(s.filterSelectableAccounts(ctx, accounts, platform, useMixed, requestedModel, excludedIDs, false))
		...
		for len(available) > 0 {
			...
			if err == nil && result.Acquired {
				...
				if !s.checkAndRegisterSession(ctx, selected.account, sessionHash) {
					...
					if s.shouldBindSelectionBeforeSuccess(ctx, selected.account, groupID, sessionHash, preserveKiroStickyBinding, deferKiroStickyMigration) && s.cache != nil {
...
func (s *GatewayService) tryAcquireByLegacyOrder(ctx context.Context, candidates []*Account, groupID *int64, sessionHash string, preferOAuth bool, preserveStickyBinding, deferStickyMigration bool) (*AccountSelectionResult, bool, error) {
	...
	for len(remaining) > 0 {
		...
		if err == nil && result.Acquired {
			...
			if s.shouldBindSelectionBeforeSuccess(ctx, acc, groupID, sessionHash, preserveStickyBinding, deferStickyMigration) && s.cache != nil {
...
func extractSystemTextAndCacheControl(system any) (string, any) {
	...
	case []any:
		...
		for _, item := range v {
			...
			if value, exists := block["cache_control"]; exists && value != nil {
				cacheControl = value
			}
		}
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (result *ForwardResult, err error) {
	...
	if shouldMimicClaudeCode {
		...
		// system 上的 cache_control 一律保留：重写时是我们拼进去的稳定锚点，
		// 未重写时（haiku / 注入开关关闭）是客户端的缓存意图；上限由 enforceCacheControlLimit 兜底。
		normalizeOpts := claudeOAuthNormalizeOptions{
			preserveBillingHeaderBlocks: systemRewritten,
		}
...
func (s *GatewayService) buildUpstreamRequestAnthropicAPIKeyPassthrough(
	...
	// base 取值同源（GetBaseURL），trailing '/' 由 helper 内部归一化，详见 helper 注释。
		...
		case ev, ok := <-events:
			...
			if data, ok := extractAnthropicSSEDataLine(line); ok {
				...
				if len(pendingEventLines) == 0 &&
					...
					writePassthroughBlock(stripSub2apiInternalUsageFields(line) + "\n")
...
func (s *GatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool) (*http.Request, []byte, error) {
	...
	// billing cc_version 与真实出站 User-Agent 同源（本地 canonical 语义，见 billingUserAgentForWire）。
	if billingUA := s.billingUserAgentForWire(ctx, tokenType, mimicClaudeCode, fingerprint); billingUA != "" {
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				...
				var clientErr clientVisibleStreamError
				if errors.As(ev.err, &clientErr) {
					if c.Writer.Written() {
						sendErrorEvent(clientErr.ClientErrorType(), clientErr.ClientErrorMessage())
					}
	...
	if apiKey.GroupID != nil {
		...
		usageLog.AccountStatsCost = resolveAccountStatsCost(
			...
			*apiKey.GroupID,
			...
			pricingAt,
			optionalStringValue(usageLog.ReasoningEffort),
		)
	}
...
func (s *GatewayService) calculateTokenCost(
	...
	if resolved := s.resolveChannelPricing(ctx, billingModel, apiKey); resolved != nil {
		...
		cost, err = s.billingService.CalculateCostUnified(CostInput{
			...
			ServiceTier: optionalStringValue(result.ServiceTier), ReasoningEffort: optionalStringValue(result.ReasoningEffort),
			Resolver: s.resolver,
		})
	} else {
...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	if shouldMimicClaudeCode {
		...
		// 4 块上限的兜底：其余出口都在自己的转发路径上调过一次，只有这里没有。
...
func (s *GatewayService) buildCountTokensRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, mimicClaudeCode bool) (*http.Request, []byte, error) {
	...
	// billing cc_version 与真实出站 User-Agent 同源（本地 canonical 语义，见 billingUserAgentForWire）。
	if billingUA := s.billingUserAgentForWire(ctx, tokenType, mimicClaudeCode, billingFingerprint); billingUA != "" {
		body = syncBillingHeaderVersion(body, billingUA)
	}
	if mimicClaudeCode {
		body = normalizeClaudeCodeMimicryUpstreamBody(body)
...
func (s *GatewayService) GetAvailableModels(ctx context.Context, groupID *int64, platform string) []string {
	// 模型清单端点（/antigravity/models、/v1beta/models 等）在没有动态清单时会回落到
	// 静态目录，调用方只看 len()==0。nil 服务应走同一条回落路径而不是打挂整个进程。
	if s == nil {
		return nil
	}
