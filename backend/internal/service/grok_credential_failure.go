...
func (s *OpenAIGatewayService) blockGrokCredentialRuntime(account *Account, until time.Time, reason string) func() {
	...
	installedBlock, isBlock := openAIAccountRuntimeBlockFromValue(installed)
	installedUntil := installedBlock.Until
	mu.Unlock()
	if !changed || !installedOK || !isBlock {
		return func() {}
	}
	if hadBefore {
		if beforeBlock, ok := openAIAccountRuntimeBlockFromValue(before); ok && beforeBlock.Until.Equal(installedUntil) {
			return func() {}
		}
	}
	return func() {
		...
		if !ok || !isBlock || !currentBlock.Until.Equal(installedUntil) {
			return
		}
		if hadBefore {
