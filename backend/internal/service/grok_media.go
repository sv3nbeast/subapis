...
// canonicalizeGrokMediaImageURLFields accepts the historic image_url alias at
// the API boundary, but sends the official url field upstream.
...
func (s *OpenAIGatewayService) handleGrokMediaErrorResponse(
	...
	if isGrokContentPolicyRejection(resp.StatusCode, body) {
		...
		writeGrokMediaErrorResponse(c, resp.StatusCode, "invalid_request_error", grokContentPolicyClientMessage(body))
...
func isGrokMediaVideoContentURL(rawURL, requestID string) bool {
	...
	return segments[len(segments)-3] == "videos" && decodedID == requestID && segments[len(segments)-1] == "content"
}
