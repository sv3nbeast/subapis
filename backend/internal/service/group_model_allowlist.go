...
func (g *Group) ModelAllowlistEnabled() bool {
	return g != nil && g.ModelAllowlist.active()
}

// active report 白名单是否真正生效：开关打开且至少有一个非空条目。
//
// 管理端写入路径已拒绝“开启但列表为空”（normalizeGroupModelAllowlist），但生产库里
// 存在改名前 models_list_config 时代留下的脏数据——当时该配置只影响 /v1/models 展示，
// 且生产的 CustomModelsListEnabled() 带有 len(Models) > 0 的 fail-open 守卫，所以
// {"enabled":true,"models":[]} 是可以被保存的。白名单升级为“同时约束请求准入”后，
// 若丢掉这个守卫，这类脏数据会让该分组的所有网关请求一律 404。此处保留守卫。
func (a GroupModelAllowlist) active() bool {
	if !a.Enabled {
		return false
	}
...
func (a GroupModelAllowlist) FilterForListing(source []string) []string {
	if !a.active() {
		// 未开启，或遗留脏数据“开启但列表为空”：与 Allows 保持一致地 fail-open，
		// 维持“看到什么 = 能调什么”。
		return source
	}
