...
func nianzsKiroSemanticTailResponse(t *testing.T, content string, percentage float64) *http.Response {
	t.Helper()
	stream := bytes.NewBuffer(nil)
	_, _ = stream.Write(kiroEventStreamFrame(t, "assistantResponseEvent", map[string]any{
...
func TestNianzsMessagesFinalizesSemanticContextUsageTailWithoutRetry(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, endpoint := range []struct {
		...
		{name: "amazon_q", mode: KiroEndpointModeQ, url: "https://q.us-east-1.amazonaws.com/generateAssistantResponse"},
		{name: "krs", mode: KiroEndpointModeKRS, url: nianzsKiroKRSEndpointURL},
	} {
		endpoint := endpoint
		for _, stream := range []bool{false, true} {
			stream := stream
			t.Run(endpoint.name+"/"+map[bool]string{false: "non_stream", true: "stream"}[stream], func(t *testing.T) {
				...
				require.Equal(t, endpoint.url, upstream.lastReq.URL.String())
				if stream {
...
func TestNianzsMessagesSemanticEOFWithoutContextUsageStaysIncomplete(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, endpoint := range []struct {
		name string
		mode string
	}{
		{name: "amazon_q", mode: KiroEndpointModeQ},
		{name: "krs", mode: KiroEndpointModeKRS},
	} {
		endpoint := endpoint
		for _, stream := range []bool{false, true} {
			stream := stream
...
func TestNianzsSemanticTailCommitsCacheForStreamingAndNonStreaming(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, endpoint := range []struct {
		name string
		mode string
		...
		for _, stream := range []bool{false, true} {
			...
			t.Run(endpoint.name+"/"+map[bool]string{false: "non_stream", true: "stream"}[stream], func(t *testing.T) {
				...
				group.ID = 29
				group.KiroEndpointMode = endpoint.mode
...
func TestNianzsChatCompletionsRouteStreamingAndNonStreaming(t *testing.T) {
	...
	for _, stream := range []bool{false, true} {
		...
		t.Run(map[bool]string{false: "non_stream", true: "stream"}[stream], func(t *testing.T) {
			...
			svc, upstream, account := newNianzsKiroRouteTestRuntime(t, nianzsKiroSemanticTailResponse(t, "nianzs chat ok", 64.2))
...
func TestNianzsResponsesRouteStreamingAndNonStreaming(t *testing.T) {
	...
	for _, stream := range []bool{false, true} {
		...
		t.Run(map[bool]string{false: "non_stream", true: "stream"}[stream], func(t *testing.T) {
			...
			c, _ := gin.CreateTestContext(recorder)
			c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(body))
			svc, upstream, account := newNianzsKiroRouteTestRuntime(t, nianzsKiroSemanticTailResponse(t, "nianzs responses ok", 64.2))
