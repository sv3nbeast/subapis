...
func TestForwardKiroMessagesNonStreamingMetadataOnlyRetriesWithFreshConversationAndEndpoint(t *testing.T) {
	...
	require.Equal(t, 11, result.Usage.InputTokens, "failed body usage must not be added to the successful retry")
...
func TestOpenKiroAnthropicStreamResponseOpus5AutoPreludeCompletesProtocolDriven(t *testing.T) {
	...
	account := &Account{
		...
		Credentials: map[string]any{
			"access_token": "test-token",
			"api_region":   "us-east-1",
			"profile_arn":  "arn:aws:codewhisperer:us-east-1:123456789012:profile/OPUS5-PRELUDE",
		},
	}
	body := []byte(`{
		"model":"claude-opus-5",
		...
		"messages":[{"role":"user","content":"back up the database before deleting data"}]
	}`)
	parsed, err := ParseGatewayRequest(NewRequestBodyRef(body), PlatformKiro)
	require.NoError(t, err)
