...
// 官方用例原以 PlatformGrok 账号验证 fail-open；本地 Grok 的配额/429/付费块在
// 持久化冷却到达调度快照之前就已装入进程内，须保留（有界 failover 延迟），
// 故 Grok 不走 isOpenAIAccountRequestRuntimeBlocked 的 fail-open 分支。
// 这里改用 PlatformOpenAI 账号验证官方语义，Grok 语义由
// TestRuntimeBlockGrokKeepsInProcessBlockWithoutPersistedCooldown 覆盖。
func TestRuntimeBlockHonorsClearedPersistedCooldown(t *testing.T) {
	svc := &OpenAIGatewayService{}
	account := &Account{ID: 92, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true}
	svc.BlockAccountScheduling(account, time.Now().Add(30*time.Minute), "oauth_401")
...
func TestRuntimeBlockConditionalClearSkipsNewerGeneration(t *testing.T) {
	svc := &OpenAIGatewayService{}
	account := &Account{ID: 94, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true}
...
// TestRuntimeBlockGrokKeepsInProcessBlockWithoutPersistedCooldown 固化本地
// Grok 语义：没有持久化冷却字段时，进程内块仍然生效（不 fail-open），
// 只由 isOpenAIAccountRuntimeBlocked 的 stale reconcile 路径按时回收。
func TestRuntimeBlockGrokKeepsInProcessBlockWithoutPersistedCooldown(t *testing.T) {
	svc := &OpenAIGatewayService{}
	account := &Account{ID: 95, Platform: PlatformGrok, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true}
