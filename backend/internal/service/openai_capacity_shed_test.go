...
func TestOpenAIStreamProcessingFailureAfterOutputIsRecorded(t *testing.T) {
	...
	for _, passthrough := range []bool{false, true} {
		t.Run(map[bool]string{false: "native", true: "passthrough"}[passthrough], func(t *testing.T) {
			...
			require.True(t, ok)
			// 已输出后的终态失败只记一条 ops 事件；本地 kind 为 stream_terminal_failed
			// （07c9017b0 起线上沿用），官方 6aabbdf54 的 stream_failed 与之重复，已去重。
			require.Len(t, events, 1)
			event := events[len(events)-1]
			require.Equal(t, "stream_terminal_failed", event.Kind)
