...
func (s *OpenAIGatewayService) forwardAsRawChatCompletions(
	...
	if account.Platform == PlatformGrok && forwardErr == nil && result != nil {
...
func (s *OpenAIGatewayService) streamRawChatCompletions(
	...
	streamTruncated := terminal.IsTruncated(clientOutputStarted)
	// xAI's raw chat stream contract terminates with [DONE]. A usage or
	// finish_reason frame without [DONE] may still be followed by billable or
	// semantic data, so do not commit Grok success on a clean early EOF.
	if account.Platform == PlatformGrok && terminal.sawDataLine && !terminal.sawDone {
		streamTruncated = true
	}
	if !clientAborted && streamTruncated {
		cause := scanErr
		if cause == nil {
			cause = fmt.Errorf("upstream stream ended before [DONE]: %w", ErrOpenAIUpstreamStreamTruncated)
		}
		...
		if account.Platform == PlatformGrok {
			return nil, newOpenAIUpstreamStreamReadError(cause)
		}
...
func (s *OpenAIGatewayService) bufferRawChatCompletions(
	...
	if account != nil && account.Platform == PlatformGrok && !gjson.ValidBytes(respBody) {
		return nil, fmt.Errorf("parse grok chat_completions response: invalid JSON")
	}
	observer := upstreamResponseModelObserverFromContext(c)
	if observer == nil {
		observer = beginUpstreamResponseModelObservation(c)
