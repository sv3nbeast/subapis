...
func TestForwardAsChatCompletions_UnknownModelDoesNotUseDefaultMappedModel(t *testing.T) {
	...
	// messagesDispatchMappedModel is an explicit /v1/messages routing result;
	// ordinary Chat Completions requests must not receive a group-default fallback.
...
func TestForwardAsResponses_KiroStoresStreamingOutputForPreviousResponseHistory(t *testing.T) {
	...
	require.NotEqual(t, "msg_stream_first", firstResult.ResponseID)
	require.NotEmpty(t, firstResult.ResponsesOutput)
	require.Contains(t, firstRec.Body.String(), "response.content_part.added")
	require.Contains(t, firstRec.Body.String(), `"text":"streamed answer"`)
	require.Contains(t, firstRec.Body.String(), `"output":[{"type":"message"`)

	secondBody := []byte(fmt.Sprintf(`{"model":"claude-sonnet-4-6","previous_response_id":%q,"input":[{"type":"input_text","text":"continue"}],"stream":false}`, firstResult.ResponseID))
	secondRec := httptest.NewRecorder()
	secondCtx, _ := gin.CreateTestContext(secondRec)
	secondCtx.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(secondBody))
	secondCtx.Request.Header.Set("Content-Type", "application/json")
