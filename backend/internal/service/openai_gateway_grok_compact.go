...
func convertOpenAICompactInputsForGrok(body []byte) ([]byte, error) {
	...
	for _, raw := range items {
		item, ok := raw.(map[string]any)
		if !ok || !isOpenAICompactionType(grokCompactStringValue(item["type"])) {
		...
		case "message":
			if content, ok := item["content"].([]any); ok {
				for _, rawContent := range content {
					...
					if text := strings.TrimSpace(grokCompactStringValue(part["text"])); text != "" {
...
func grokCompactStringValue(value any) string {
	text, _ := value.(string)
	return text
}
