...
// openAICompatBufferedReadError marks failures while reading an upstream body;
// the endpoint decides whether replaying that request is safe.
...
func (s *OpenAIGatewayService) handleAnthropicBufferedStreamingResponse(
	...
	anthropicResp, _ = normalizeOpenAICompatAnthropicResponse(anthropicResp)
...
func (s *OpenAIGatewayService) handleAnthropicStreamingResponse(
	...
	processDataLine := func(payload, eventTypeHint string) bool {
		...
		isTerminalEvent := isOpenAICompatResponsesTerminalEvent(eventType) || eventType == "error"
		if isTerminalEvent {
		...
		events = xmlInvokeBridge.normalizeEvents(events)
		if !clientDisconnected {
	...
	finalizeStream := func() (*OpenAIForwardResult, error) {
		...
		finalEvents := xmlInvokeBridge.normalizeEvents(apicompat.FinalizeResponsesAnthropicStream(state))
