...
func TestOpenAIImagesResponsesDriverAndImageModels(t *testing.T) {
	// 默认驱动模型引用本地常量 openAIImagesResponsesMainModel（本地 gpt-5.6-sol，
	// 2026-09-16 live 实测；官方为 gpt-5.6-luna），显式 override 用例取一个与默认不同的值。
	for _, override := range []string{"", "  ", " gpt-5.6-luna "} {
