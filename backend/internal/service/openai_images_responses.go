...
func writeOpenAIImagesUpstreamErrorResponse(c *gin.Context, err *OpenAIImagesUpstreamError) bool {
	...
	// A JSON keepalive commits only leading JSON whitespace. It must not prevent
	// the actual upstream error document from being written afterwards.
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthResponseError(
	...
	return &UpstreamFailoverError{
		...
		RetryableOnSameAccount: shouldRetryOpenAIAccountOnSameAccount(account, upstreamErr.StatusCode, responseBody, false),
	}
}
