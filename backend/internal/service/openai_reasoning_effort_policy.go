...
func normalizeAdminGroupReasoningPolicy(group *Group) error {
	if group == nil {
		return nil
	}
	if group.Platform != PlatformAnthropic && group.Platform != PlatformOpenAI && group.Platform != PlatformComposite {
	...
	maxEffort, err := normalizeMaxReasoningEffortForPlatform(group.Platform, group.MaxReasoningEffort)
	if err != nil {
		return err
	}
	mappings, err := NormalizeReasoningEffortMappings(group.Platform, group.ReasoningEffortMappings)
	if err != nil {
		return err
	}
	if mappings == nil {
