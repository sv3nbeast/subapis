...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	routingServiceTier := ""
	if len(extra) > 0 {
		routingModel = extra[0]
	}
	if len(extra) > 1 {
		routingServiceTier = extra[1]
	}
...
func normalizeOpenAIWSPayloadWithoutInputAndPreviousResponseID(payload []byte) ([]byte, error) {
	...
	// Codex changes transport-only metadata for every response.create. These
	// fields do not alter the context referenced by previous_response_id.
	delete(decoded, "client_metadata")
	delete(decoded, "stream_options")
	// Codex may prewarm with generate=false then omit it on the business
	// request. Preserve generate=true because that is a semantic change.
...
func buildOpenAIWSReplayInputSequenceFromItems(
	...
	if openAIWSRawItemsHasPrefix(currentItems, previousFullInput) ||
		openAIWSRawItemsContainsInOrder(currentItems, previousFullInput) {
...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	wsHeaders, sessionResolution, buildHdrErr := s.buildOpenAIWSHeaders(
		ctx, c, account, token, decision, isCodexCLI, turnState, turnMetadata, promptCacheKey,
	...
	beat := func() error {
		...
		if err != nil {
			markClientDisconnected("keepalive_write_error")
			return err
		}
		flusher.Flush()
		lastDownstreamWriteAt = time.Now()
...
readLoop:
	for {
		...
		if readErr != nil {
			...
			// 客户端自身断开/取消不触发账号 failover：请求 ctx 取消导致的读失败先换
			// 分离 ctx 继续排空一次，其余情况直接结束；输出前 failover 边界仅对仍在线的客户端生效。
			if clientDisconnected {
		...
		if isTerminalEvent {
			...
			// A terminal event must be the last JSON document of this message. Do
			// not forward a trailing ambiguous document, and never return this
			// connection to the pool when one exists.
			cleanExit = len(pendingJSONDocuments) == 0
			break
		}
	}

	if clientDisconnected && terminalEventCount == 0 && !nativeCompactionStream {
		return buildForwardResult(), fmt.Errorf("openai ws stream incomplete after client disconnect: %w", context.Canceled)
	}
	if !reqStream {
		if clientDisconnected && !nativeCompactionStream {
...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	// The handler normally owns this registration across retry attempts (it
	// passes the client conn so the superseded connection receives a close
	// frame before cancellation). Direct callers still get the same
	// execution-scope preemption here; a context already registered by the
	// handler is reused as-is.
	...
	parseClientPayload := func(raw []byte, turn int) (openAIWSClientPayload, error) {
		...
		accountScopedPayload, accountScoped, scopeErr := applyCodexAccountIdentityClientMetadataRaw(
			normalized,
		...
		// The ChatGPT OAuth WebSocket endpoint is Responses-Lite, which rejects
		// hosted image_generation tools. Do not auto-inject one on this path.
		codexBridgeEnabled := isCodexCLI && !codexResponsesLite && imageGenerationAllowed && codexImageGenerationExplicitToolPolicy != codexImageGenerationExplicitToolPolicyStrip && s.isCodexImageGenerationBridgeEnabled(ctx, account, apiKey)
		...
		for turn := 1; ; turn++ {
			...
			if account.Platform == PlatformGrok {
				grokCacheIdentity, err = resolveGrokWSCacheIdentity(
					c, account, firstPayload.payloadRaw, currentBridgePayload.payloadRaw, currentBridgePayload.originalModel,
				)
				if err != nil {
	...
	wsHeaders, _, buildHdrErr := s.buildOpenAIWSHeaders(
		ctx, c, account, token, wsDecision, isCodexCLI, turnState,
		strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader)), firstPayload.promptCacheKey,
		firstRoutingFields[0].String(), firstRoutingFields[1].String(),
	)
	if buildHdrErr != nil {
	...
	for {
		...
		if nextPayload.promptCacheKey != "" {
			...
			updatedHeaders, _, updHdrErr := s.buildOpenAIWSHeaders(
				ctx, c, account, token, wsDecision, isCodexCLI, turnState,
				strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader)), nextPayload.promptCacheKey,
				nextRoutingFields[0].String(), nextRoutingFields[1].String(),
			)
			if updHdrErr != nil {
...
func (s *OpenAIGatewayService) SelectAccountByPreviousResponseID(
	...
	// Public previous_response_id selection bypasses the ordinary scheduler, so it
	// must install the same request gate before testing the sticky account.
...
func (s *OpenAIGatewayService) selectAccountByPreviousResponseIDForCapability(
	...
	// survive an HTTP fallback. API-key Responses HTTP continuation is scoped to
	// the selected key/project, so it must retain the account that created it.
	...
	// Profit eligibility is transient just like quota pause: skip this reuse but
	// retain the response binding so a recovered account can resume the chain.
...
func (s *OpenAIGatewayService) newOpenAIWSRateLimitFailoverError(account *Account, headers http.Header, responseBody []byte, message string) *UpstreamFailoverError {
	return s.newOpenAIAccountFailoverError(
		account, http.StatusTooManyRequests, headers, responseBody,
		strings.TrimSpace(message), false, false,
	)
}
