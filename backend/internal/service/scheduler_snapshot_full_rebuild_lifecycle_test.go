...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	require.Len(t, cache.tokens(), 2*canonicalTestBucketCount, "full rebuild and the following group event must each run fresh authority")
	_, reopenHeld := cache.lifecycleMutationLeaseStates()
	require.Len(t, reopenHeld, 2*canonicalTestBucketCount)
	for _, held := range reopenHeld {
		require.True(t, held)
	}
	require.Equal(t, 3*canonicalTestQueryCount, accounts.callCount())
}
...
func TestSchedulerFullRebuildFreshActivePreparesEveryTokenBeforeFirstDB(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.Equal(t, canonicalTestBucketCount, reopenCount)
		require.Equal(t, canonicalTestBucketCount+1, capturesAtFirstDB, "C(0) and the historical bucket must be captured before DB")
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)
	...
	require.Equal(t, 2*canonicalTestQueryCount+1, accounts.callCount())
	require.Equal(t, canonicalTestQueryCount+1, accounts.groupCallCount(groupID))
...
func TestSchedulerFullRebuildOrdinaryCaptureErrorReturnsBeforeFirstDB(t *testing.T) {
	...
	require.Equal(t, canonicalTestBucketCount+2, cache.captureAttemptCount(), "all canonical and ordinary captures must be attempted before returning")
...
func TestSchedulerFullRebuildPreservesGroupZeroActiveHistoricalAndInvalidRegistryBuckets(t *testing.T) {
	...
	require.NoError(t, svc.rebuildFullSnapshot(context.Background(), "test"))
	require.Equal(t, 2*canonicalTestBucketCount+3, cache.captureAttemptCount())
	require.Equal(t, 2*canonicalTestQueryCount+3, accounts.callCount())
	groups.mu.Lock()
	require.Equal(t, 1, groups.listCalls)
	groups.mu.Unlock()
...
func TestSchedulerFullRebuildActiveTombstoneFreshInactiveOrMissingFiltersAllGroupTasks(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, canonicalTestQueryCount, accounts.groupCallCount(0))
			require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Zero(t, accounts.callCount())
	require.Zero(t, cache.totalSetAttempts())
	require.Equal(t, canonicalTestBucketCount+1, len(cache.retiredBuckets()))
	...
	require.Equal(t, 3*(canonicalTestBucketCount+1), len(cache.retiredBuckets()))
	require.Equal(t, canonicalTestQueryCount, accounts.callCount())
	require.Empty(t, cache.tokens())
}
...
func TestSchedulerFullRebuildActiveTombstoneLazyRecoveryDiscardsPartialCaptureTasks(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.False(t, held)
		require.Equal(t, canonicalTestBucketCount, reopenCount)
		require.Equal(t, canonicalTestBucketCount+7, capturesAtFirstDB)
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Zero(t, fallbackCalls)
	require.Empty(t, freshCalls)
	require.Equal(t, canonicalTestBucketCount+3, cache.captureAttemptCount())
	require.Equal(t, canonicalTestQueryCount+3, accounts.callCount())
	require.Equal(t, canonicalTestQueryCount+3, accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	require.Equal(t, 2*canonicalTestQueryCount, accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, 4*canonicalTestQueryCount, accounts.callCount())
