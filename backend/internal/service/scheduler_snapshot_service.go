...
// OpenAI uses this for the guarded Kiro text bridge without changing normal
// OpenAI scheduling behavior.
...
func (s *SchedulerSnapshotService) listSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform, useMixed bool) ([]Account, bool, error) {
...
func (s *SchedulerSnapshotService) rebuildByAccount(ctx context.Context, account *Account, groupIDs []int64, reason string, seen map[batchSeenKey]struct{}) error {
	...
	if account.SupportsMixedScheduling() {
		...
		if account.Platform == PlatformKiro {
			buckets = append(buckets, s.bucketsForPlatform(PlatformOpenAI, groupIDs, seen)...)
		}
	}
...
func schedulerSnapshotPlatforms() [12]string {
	// Kiro intentionally uses its separate scheduler, but CN and Cursor
	// accounts need normal snapshot rebuilds after account/group changes.
	return [12]string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformDroid, PlatformGrok, PlatformCursor, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformOpenCodeGo}
}
...
func schedulerCanonicalBuckets(groupID int64) []SchedulerBucket {
	...
	buckets := make([]SchedulerBucket, 0, len(platforms)*2+3)
...
func (s *SchedulerSnapshotService) loadAccountsFromDB(ctx context.Context, bucket SchedulerBucket, useMixed bool) ([]Account, error) {
	...
	if useMixed {
		platforms := mixedSchedulingQueryPlatforms(bucket.Platform)
		...
		for _, acc := range accounts {
			if !isAccountAllowedInMixedScheduling(&acc, bucket.Platform) {
				continue
			}
