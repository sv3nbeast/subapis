...
// 已过期 ResetUsageForQuotaCycle，0b2a61d92 与 bf1e0bc3a）；官方桩缺这两个方法会
// 走到 nil 内嵌接口。它们属于同一次 extend 的附属写入，不计入 mutations。
func (r *bulkActionSubscriptionRepo) SetQuotaCycle(_ context.Context, id int64, startAt, endAt time.Time, cycleDays int) error {
	sub := r.subscriptions[id]
	if sub == nil {
...
func (r *bulkActionSubscriptionRepo) ResetUsageForQuotaCycle(_ context.Context, id int64, windowStart, cycleStartAt, cycleEndAt time.Time, cycleDays int) error {
	sub := r.subscriptions[id]
	if sub == nil {
		return ErrSubscriptionNotFound
	}
