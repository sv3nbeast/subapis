...
// 本地 ExtendSubscription 在同一事务内同步配额周期（未过期 SetQuotaCycle /
// 已过期 ResetUsageForQuotaCycle，0b2a61d92 与 bf1e0bc3a）；与其它写入一样只暂存到
// 当前事务的 pending 副本，随真实提交/回滚一起生效或丢弃。
func (r *transactionalBulkSubscriptionRepo) SetQuotaCycle(ctx context.Context, _ int64, startAt, endAt time.Time, cycleDays int) error {
	sub := r.pending[dbent.TxFromContext(ctx)]
	sub.QuotaCycleStartAt, sub.QuotaCycleEndAt, sub.QuotaCycleDays = &startAt, &endAt, cycleDays
	return nil
}

func (r *transactionalBulkSubscriptionRepo) ResetUsageForQuotaCycle(ctx context.Context, _ int64, windowStart, cycleStartAt, cycleEndAt time.Time, cycleDays int) error {
	sub := r.pending[dbent.TxFromContext(ctx)]
	sub.DailyUsageUSD, sub.WeeklyUsageUSD, sub.MonthlyUsageUSD = 0, 0, 0
	sub.DailyWindowStart, sub.WeeklyWindowStart, sub.MonthlyWindowStart = &windowStart, &windowStart, &windowStart
	sub.QuotaCycleStartAt, sub.QuotaCycleEndAt, sub.QuotaCycleDays = &cycleStartAt, &cycleEndAt, cycleDays
	return nil
}
