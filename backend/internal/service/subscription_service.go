...
func (s *SubscriptionService) runInTx(ctx context.Context, fn func(context.Context) error) error {
...
// updateExistingSubscriptionTerm bases every renewal decision on the current
// locked record instead of the caller's potentially stale snapshot.  Assignment
// flows keep their suspended-subscription and semantic-note idempotency rules.
func (s *SubscriptionService) updateExistingSubscriptionTerm(
	...
	return s.runInTx(ctx, func(txCtx context.Context) error {
		...
		if isExpired {
			...
			// Preserve the local quota-cycle contract for subscriptions that use
			// quota-cycle accounting, while leaving legacy subscriptions on the
			// upstream-compatible renewal path.
			if existingSub.QuotaCycleStartAt != nil || existingSub.QuotaCycleEndAt != nil || existingSub.QuotaCycleDays > 0 {
				if err := s.userSubRepo.SetQuotaCycle(txCtx, existingSub.ID, now, newExpiresAt, validityDays); err != nil {
					return fmt.Errorf("set quota cycle: %w", err)
				}
			}
			return nil
		}
		...
		if existingSub.QuotaCycleStartAt != nil || existingSub.QuotaCycleEndAt != nil || existingSub.QuotaCycleDays > 0 {
			cycleStart, cycleEnd, cycleDays := currentQuotaCycleForActiveRenewal(existingSub, validityDays, newExpiresAt)
			if err := s.userSubRepo.SetQuotaCycle(txCtx, existingSub.ID, cycleStart, cycleEnd, cycleDays); err != nil {
				return fmt.Errorf("set quota cycle: %w", err)
			}
		}
...
func (s *SubscriptionService) createSubscription(ctx context.Context, input *AssignSubscriptionInput) (*UserSubscription, error) {
	...
	sub := &UserSubscription{
		...
		QuotaCycleEndAt:   &expiresAt,
		QuotaCycleDays:    validityDays,
		AssignedAt:        now,
		Notes:             input.Notes,
		CreatedAt:         now,
		UpdatedAt:         now,
...
func renewedSubscriptionTerm(existingSub *UserSubscription, notes string, startsAt, expiresAt time.Time) *UserSubscription {
	...
	renewed.ModelUsage = map[string]SubscriptionModelUsage{}
...
func currentQuotaCycleForActiveRenewal(sub *UserSubscription, validityDays int, newExpiresAt time.Time) (time.Time, time.Time, int) {
	cycleDays := normalizeAssignValidityDays(validityDays)
	if sub == nil {
		now := time.Now()
		return now, subscriptionMinTime(now.AddDate(0, 0, cycleDays), newExpiresAt), cycleDays
	}

	startAt := sub.StartsAt
	if sub.QuotaCycleStartAt != nil {
		startAt = *sub.QuotaCycleStartAt
	}
	if startAt.IsZero() {
		startAt = time.Now()
	}
	...
	endAt := sub.ExpiresAt
	if sub.QuotaCycleEndAt != nil {
		endAt = *sub.QuotaCycleEndAt
	}
	if endAt.IsZero() || endAt.After(sub.ExpiresAt) {
		endAt = sub.ExpiresAt
	}
	if endAt.After(newExpiresAt) {
		endAt = newExpiresAt
	}
	if !endAt.After(startAt) {
		endAt = subscriptionMinTime(startAt.AddDate(0, 0, cycleDays), newExpiresAt)
	}

	if sub.QuotaCycleDays > 0 {
		cycleDays = normalizeAssignValidityDays(sub.QuotaCycleDays)
	}
	return startAt, endAt, cycleDays
}

func subscriptionMinTime(a, b time.Time) time.Time {
	if a.After(b) {
		return b
	}
	return a
...
func adjustedQuotaCycleAfterActiveExpiryChange(sub *UserSubscription, newExpiresAt time.Time, now time.Time) (time.Time, time.Time, int) {
	cycleDays := 30
	if sub != nil {
		cycleDays = sub.EffectiveQuotaCycleDays()
	}
	...
	endAt := newExpiresAt
	if sub != nil && sub.QuotaCycleEndAt != nil {
		endAt = *sub.QuotaCycleEndAt
	}
	if endAt.After(newExpiresAt) {
		endAt = newExpiresAt
...
func (s *SubscriptionService) ExtendSubscription(ctx context.Context, subscriptionID int64, days int) (*UserSubscription, error) {
	...
	if err := s.runInTx(ctx, func(txCtx context.Context) error {
		...
		if isExpired {
			...
			if err := s.userSubRepo.ResetUsageForQuotaCycle(txCtx, subscriptionID, windowStart, now, newExpiresAt, cycleDays); err != nil {
				return err
			}
		} else {
			cycleStart, cycleEnd, cycleDays := adjustedQuotaCycleAfterActiveExpiryChange(sub, newExpiresAt, now)
			if err := s.userSubRepo.SetQuotaCycle(txCtx, subscriptionID, cycleStart, cycleEnd, cycleDays); err != nil {
				return err
			}
		}
		// 如果订阅已过期，恢复为active状态
...
func (s *SubscriptionService) checkAndActivateWindowAt(ctx context.Context, sub *UserSubscription, now time.Time) error {
	...
	case interface {
		...
		err = repo.ActivateWindows(ctx, sub.ID, dailyStart, periodicStart)
	case interface {
		ActivateWindows(context.Context, int64, time.Time) error
	}:
		err = repo.ActivateWindows(ctx, sub.ID, dailyStart)
	default:
		return nil
	}
	if err != nil {
...
func (s *SubscriptionService) CheckAndResetWindows(ctx context.Context, sub *UserSubscription) error {
	...
	if sub.NeedsQuotaCycleResetAt(now) {
		cycleStart, cycleEnd, cycleDays := sub.NextQuotaCycleAfter(now)
		if err := s.userSubRepo.ResetUsageForQuotaCycle(ctx, sub.ID, dailyStart, cycleStart, cycleEnd, cycleDays); err != nil {
			return err
		}
		sub.DailyWindowStart = &dailyStart
		sub.WeeklyWindowStart = &cycleStart
		sub.MonthlyWindowStart = &cycleStart
		sub.QuotaCycleStartAt = &cycleStart
		sub.QuotaCycleEndAt = &cycleEnd
	...
	if !needsInvalidateCache {
		...
		if windowStart, ok := sub.automaticWindowStartAt(sub.WeeklyWindowStart, 7*24*time.Hour, periodicStart); ok {
		...
		if windowStart, ok := sub.automaticWindowStartAt(sub.MonthlyWindowStart, 30*24*time.Hour, periodicStart); ok {
...
func (s *SubscriptionService) EnsureWindowMaintenance(ctx context.Context, sub *UserSubscription) (*UserSubscription, error) {
	...
	value, err, _ := s.maintGroup.Do(key, func() (any, error) {
		...
		now := s.now()
		windowStart := startOfDay(now)
		if current.NeedsQuotaCycleResetAt(now) {
			cycleStart, cycleEnd, cycleDays := current.NextQuotaCycleAfter(now)
			if err := s.userSubRepo.ResetUsageForQuotaCycle(ctx, current.ID, windowStart, cycleStart, cycleEnd, cycleDays); err != nil {
				return nil, err
			}
			changed = true
		} else if resetRepo, ok := s.userSubRepo.(conditionalWindowResetRepository); ok {
