...
type TokenRefreshService struct {
	...
	// Legacy test seam retained for direct service assembly. Production uses registrations.
...
// NewTokenRefreshServiceWithKiro extends the official constructor with the
// local Kiro provider while keeping the upstream constructor source-compatible.
func NewTokenRefreshServiceWithKiro(
	accountRepo AccountRepository,
	oauthService *OAuthService,
	openaiOAuthService *OpenAIOAuthService,
	geminiOAuthService *GeminiOAuthService,
...
func newTokenRefreshService(
	...
	if kiroOAuthService != nil {
		s.registrations = append(s.registrations[:4], append([]tokenRefreshRegistration{{platform: PlatformKiro, refresher: kiroRefresher, executor: kiroRefresher}}, s.registrations[4:]...)...)
	}

	return s
}

func (s *TokenRefreshService) ensureLegacyRegistrations() {
	if s == nil || len(s.registrations) > 0 || len(s.refreshers) == 0 {
		return
	}
	platforms := []string{
		PlatformAnthropic,
	...
	for _, platform := range platforms {
		for i, refresher := range s.refreshers {
			...
			} else if typed, ok := refresher.(OAuthRefreshExecutor); ok {
				executor = typed
			}
...
func (s *TokenRefreshService) processRefreshContext(parent context.Context) {
	...
	s.ensureLegacyRegistrations()
	if pager == nil {
		if legacy, ok := s.accountRepo.(interface {
			ListOAuthRefreshCandidates(context.Context) ([]Account, error)
		}); ok {
			accounts, err := legacy.ListOAuthRefreshCandidates(ctx)
			if err != nil {
			...
			s.processLegacyRefreshCandidates(ctx, accounts, refreshWindow)
			return
		}
	...
	providerStates := s.buildTokenRefreshProviderStates()
...
func (s *TokenRefreshService) processLegacyRefreshCandidates(ctx context.Context, accounts []Account, refreshWindow time.Duration) {
	for i := range accounts {
		...
		for _, registration := range s.registrations {
			...
			if registration.refresher.NeedsRefresh(account, refreshWindow) {
				_ = s.refreshWithRetry(ctx, account, registration.refresher, registration.executor, refreshWindow)
			}
			break
		}
	}
...
func (s *TokenRefreshService) buildTokenRefreshProviderStates() map[string]*tokenRefreshProviderState {
...
func (s *TokenRefreshService) postRefreshActions(ctx context.Context, account *Account) {
	...
	// 只清除 token refresh 自己设置的临时不可调度状态；quota/manual 隔离必须保留。
