...
func TestTokenRefreshService_RefreshWithRetry_PrefillsKiroProfileArn(t *testing.T) {
	account := &Account{
		ID:          9,
	...
	upstream := &tokenRefreshKiroProfileUpstream{
	...
	require.Equal(t, "arn:aws:codewhisperer:us-east-1:123456789012:profile/RESOLVED", repo.accountsByID[account.ID].GetCredential("profile_arn"))
}

type tokenRefreshKiroProfileUpstream struct {
	responses []*http.Response
	requests  []*http.Request
}

func (u *tokenRefreshKiroProfileUpstream) Do(*http.Request, string, int64, int) (*http.Response, error) {
	return nil, errors.New("unexpected Do call")
}

func (u *tokenRefreshKiroProfileUpstream) DoWithTLS(req *http.Request, _ string, _ int64, _ int, _ *tlsfingerprint.Profile) (*http.Response, error) {
...
func TestIsNonRetryableRefreshError(t *testing.T) {
	tests := []struct {
		...
		{name: "kiro_empty_refresh_token", err: errors.New("kiro refresh_token is empty; reauthorize Kiro account"), expected: true},
