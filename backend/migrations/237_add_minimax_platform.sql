...
-- 本地 fork：user_platform_quotas 已含 kiro/droid（147/148/234）与 cursor（239）行，
-- 重建约束必须是本地既有集合的超集，否则 ADD CONSTRAINT 校验存量行失败、迁移整体回滚。
ALTER TABLE user_platform_quotas
    ADD CONSTRAINT user_platform_quotas_platform_check
    CHECK (platform IN ('anthropic', 'openai', 'gemini', 'antigravity', 'kiro', 'droid', 'grok',
                        'cursor', 'kimi', 'zhipu', 'deepseek', 'minimax'));
