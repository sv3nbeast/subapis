...
def validate_thinking_signature(
    value: Any,
    field: str,
    expected_issuer: str = "kiro",
) -> dict[str, Any]:
    require(expected_issuer in {"kiro", "claude"}, f"unsupported thinking signature issuer: {expected_issuer}")
    ...
    # Kiro-issued signatures carry channel field 2=1. Native Anthropic Claude
    # signatures deliberately omit it. This distinction lets the probe verify
    # that shadow mode returned the real Claude envelope rather than merely a
    # Kiro envelope with a compatible outer shape.
    if expected_issuer == "kiro":
        require(channel.get(2) == (0, 1), f"{field} is not a provider-issued Kiro envelope")
    else:
        require(2 not in channel, f"{field} is not a native Claude envelope")
...
def validate_adaptive_thinking_progress(facts: dict[str, Any]) -> None:
    """Validate stable Claude adaptive-thinking invariants.

    Anthropic can emit a different number of estimated-token progress deltas
    for otherwise identical requests. Values observed on the native Claude
    route are positive multiples of 50, followed by one final null estimate
    and then exactly one signature delta. The count and intermediate values
    are telemetry, not a deterministic part of the signed envelope.
    """
    progress = facts.get("thinking_progress")
    require(isinstance(progress, list) and bool(progress), "adaptive thinking emitted no progress deltas")
    require(
        facts.get("thinking_delta_count") == len(progress),
        "adaptive thinking progress count is inconsistent",
    )
    require(facts.get("signature_delta_count") == 1, "adaptive thinking must emit exactly one signature delta")
    require(progress[-1] is None, f"adaptive thinking final progress is not null: {progress!r}")
    for value in progress[:-1]:
        require(
            isinstance(value, int) and not isinstance(value, bool) and value > 0 and value % 50 == 0,
            f"adaptive thinking progress is invalid: {progress!r}",
        )
...
def run_probe(client: Client, model: str, signature_issuer: str = "kiro") -> list[dict[str, Any]]:
    ...
    def thinking_stream() -> dict[str, Any]:
        ...
        validate_adaptive_thinking_progress(facts)
        envelopes: list[dict[str, Any]] = []
        for signature in facts.pop("signature_values"):
            envelope = validate_thinking_signature(signature, "thinking signature", signature_issuer)
            require(envelope["provider_native"], f"thinking signature is not a native {signature_issuer} envelope")
    ...
    def signature_replay() -> dict[str, Any]:
        ...
        for block in thinking_blocks:
            ...
            envelope = validate_thinking_signature(block.get("signature"), "replay thinking signature", signature_issuer)
            require(envelope["provider_native"], f"replay signature is not a native {signature_issuer} envelope")
            envelopes.append(envelope)
...
def main() -> int:
    ...
    parser.add_argument(
        "--signature-issuer",
        choices=("kiro", "claude"),
        default=os.getenv("SUB2API_EXPECTED_SIGNATURE_ISSUER", "kiro"),
        help="expected thinking signature envelope issuer (use claude for shadow replacement mode)",
    )
    parser.add_argument(
        "--user-agent",
    ...
    try:
        ...
        checks = run_probe(client, model, args.signature_issuer)
