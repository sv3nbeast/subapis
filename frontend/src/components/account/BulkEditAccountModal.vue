    ...
    <form id="bulk-edit-account-form" class="space-y-5" @submit.prevent="() => handleSubmit()">
      ...
      <div v-if="allOpenAIPassthroughCapable" class="border-t border-gray-200 pt-4 dark:border-dark-600">
        <div class="mb-3 flex items-center justify-between gap-4">
          <div class="flex-1">
            <label id="bulk-edit-openai-long-context-billing-label" class="input-label mb-0" for="bulk-edit-openai-long-context-billing-enabled">
        ...
        >
          ...
          >
            <span :class="['inline-block h-5 w-5 rounded-full bg-white shadow transition-transform', openAILongContextBillingEnabled ? 'translate-x-5' : 'translate-x-0']" />
          </button>
        </div>
        <p class="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:bg-amber-900/20 dark:text-amber-300" data-testid="bulk-edit-openai-long-context-shadow-hint">
      ...
      <div v-if="allOpenAIAPIKey" class="border-t border-gray-200 pt-4 dark:border-dark-600">
        <div class="mb-3 flex items-center justify-between gap-4">
          <div class="flex-1">
            <label id="bulk-edit-openai-endpoint-capabilities-label" class="input-label mb-0" for="bulk-edit-openai-endpoint-capabilities-enabled">
              {{ t('admin.accounts.openai.endpointCapabilities') }}
            </label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.openai.endpointCapabilitiesDesc') }}</p>
          </div>
          <input
        ...
        <div id="bulk-edit-openai-endpoint-capabilities-body" :class="!enableOpenAIEndpointCapabilities && 'pointer-events-none opacity-50'">
          <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
            <label v-for="option in openAIEndpointCapabilityOptions" :key="option.value" class="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm dark:border-dark-600">
              <input
                type="checkbox"
      ...
      <div v-if="allOpenAIAPIKey" class="border-t border-gray-200 pt-4 dark:border-dark-600">
        <div class="mb-3 flex items-center justify-between gap-4">
          <div class="flex-1">
            <label id="bulk-edit-openai-responses-mode-label" class="input-label mb-0" for="bulk-edit-openai-responses-mode-enabled">
              {{ t('admin.accounts.openai.responsesMode') }}
            </label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.openai.responsesModeDesc') }}</p>
          </div>
          <input
        ...
        <div id="bulk-edit-openai-responses-mode-body" :class="!enableOpenAIResponsesMode && 'pointer-events-none opacity-50'">
          <Select
