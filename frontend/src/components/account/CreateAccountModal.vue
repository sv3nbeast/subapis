<template>
  <BaseDialog :show="show" :title="t('admin.accounts.createAccount')" width="wide" @close="handleClose">
    ...
    <form v-if="step === 1" id="create-account-form" @submit.prevent="handleSubmit" class="space-y-5">
      ...
      <!-- 国产供应商账号模式与协议是两个独立维度；OpenCode 只有协议维度（套餐在上方选择）。 -->
      <div v-if="isMultiProtocolPlatform" class="space-y-4">
        <div v-if="!isOpenCodeGoPlatform">
          <label class="input-label">{{ t('admin.accounts.cnProviders.accountMode.title') }}</label>
          <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <button
              ...
              :class="[
                'flex items-center gap-3 rounded-lg border-2 p-3 text-left transition-all',
                accountMode === 'payg' ? cnAccentActiveClass : 'border-gray-200 dark:border-dark-600'
              ]"
            >
              <span :class="['flex h-8 w-8 items-center justify-center rounded-lg', accountMode === 'payg' ? cnAccentIconClass : 'bg-gray-100 text-gray-500 dark:bg-dark-600']">
                <Icon name="creditCard" size="sm" />
              </span>
              <span>
                <span class="block text-sm font-medium">{{ t('admin.accounts.cnProviders.accountMode.payg') }}</span>
                <span class="text-xs text-gray-500">{{ t('admin.accounts.cnProviders.accountMode.paygDesc') }}</span>
              </span>
            </button>
            <button
              ...
              :class="[
                'flex items-center gap-3 rounded-lg border-2 p-3 text-left transition-all',
                accountMode === 'coding' ? cnAccentActiveClass : 'border-gray-200 dark:border-dark-600'
              ]"
            >
              <span :class="['flex h-8 w-8 items-center justify-center rounded-lg', accountMode === 'coding' ? cnAccentIconClass : 'bg-gray-100 text-gray-500 dark:bg-dark-600']">
                <Icon name="bolt" size="sm" />
              </span>
              <span>
                <span class="block text-sm font-medium">{{ t('admin.accounts.cnProviders.accountMode.coding') }}</span>
                <span class="text-xs text-gray-500">{{ t('admin.accounts.cnProviders.accountMode.codingDesc') }}</span>
              </span>
            </button>
          </div>
        </div>
          ...
          <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <button
              ...
              :class="[
                ...
                apiProtocol === option.value ? cnAccentActiveClass : 'border-gray-200 dark:border-dark-600'
              ]"
            >
              <span class="block text-sm font-medium">{{ t(`admin.accounts.cnProviders.apiProtocol.${option.labelKey}`) }}</span>
              <span class="mt-1 block text-xs text-gray-500">{{ t(`admin.accounts.cnProviders.apiProtocol.${option.labelKey}Desc`) }}</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="form.platform === 'zhipu' && accountMode === 'coding'" class="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input v-model="zhipuOrganization" type="text" class="input" />
          </div>
          <div>
            <label class="input-label">{{ t('admin.accounts.cnProviders.zhipuTeam.project') }}</label>
            <input v-model="zhipuProject" type="text" class="input" />
          </div>
        </div>
      </div>
      ...
      <!-- Kiro only supports model mapping mode. -->
      ...
      <!-- OpenAI Codex namespace tool flattening (OAuth relay compatibility only) -->
      <div
...
const canExchangeCode = computed(() => {
  ...
  if (form.platform === 'droid') {
    return droidOAuth.sessionId.value && !droidOAuth.loading.value
  }
...
const getKiroOAuthErrorMessage = (error: any) =>
  error?.message ||
  error?.response?.data?.message ||
  error?.response?.data?.detail ||
  t('admin.accounts.oauth.authFailed')

// Kiro OAuth / IDC 授权码兑换
const handleKiroExchange = async (authCode: string) => {
  ...
  if (loginOption === 'awsidc') {
    ...
    kiroAccountType.value = 'idc'
    kiroIDCStartUrl.value = issuerURL
    if (idcRegion) {
      kiroIDCRegion.value = idcRegion
    }
