  ...
  <BaseDialog :show="show" :title="t('admin.accounts.editAccount')" width="normal" @close="handleClose">
    <form v-if="account" id="edit-account-form" @submit.prevent="handleSubmit" class="space-y-5">
      ...
      >
        ...
        <template v-else>
          <!-- Mode Toggle -->
          <div v-if="account.platform === 'openai'" class="mb-4 flex gap-2">
            <button
              type="button"
              @click="modelRestrictionMode = 'whitelist'"
              :class="[
              ...
              >
                <div class="flex items-center gap-2">
                  <input
                    ...
                    :class="[
                      'input flex-1',
                      account.platform === 'kiro' && !isValidWildcardPattern(mapping.from)
                  ...
                  <input
                    ...
                    :class="[
                      'input flex-1',
                      account.platform === 'kiro' && mapping.to.includes('*')
                        ? 'border-red-500 dark:border-red-500'
                        : ''
                    ]"
                    :placeholder="t('admin.accounts.actualModel')"
      ...
      <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <div>
          ...
          />
          <p class="input-hint">
            {{ t(upstreamBillingRateSyncEnabled
      ...
      >
        <div class="flex items-center justify-between gap-4">
          ...
          >
            <span :class="['inline-block h-5 w-5 rounded-full bg-white shadow transition-transform', autoResetCreditEnabled ? 'translate-x-5' : 'translate-x-0']" />
          </button>
        </div>
        <div class="grid gap-4 sm:grid-cols-2">
          <div>
            <label class="input-label">{{ t('admin.accounts.autoResetCredit.threshold5h') }}</label>
            <input v-model.number="autoResetCredit5hThreshold" type="number" min="0.1" max="100" step="0.1" class="input" :disabled="!autoResetCreditEnabled" data-testid="auto-reset-credit-5h-threshold" />
          </div>
          <div>
            <label class="input-label">{{ t('admin.accounts.autoResetCredit.threshold7d') }}</label>
            <input v-model.number="autoResetCredit7dThreshold" type="number" min="0.1" max="100" step="0.1" class="input" :disabled="!autoResetCreditEnabled" data-testid="auto-reset-credit-7d-threshold" />
          </div>
        </div>
...
// 所选分组全部启用长上下文定价时，账号级长上下文开关无意义，隐藏之（官方行为）。
...
// 国产供应商账号允许修正账号模式、协议和各协议端点。回填期间暂停 watcher，
// 防止用户保存的自定义地址被默认值覆盖。
...
const cnProtocolDescKey = computed(
  () => `admin.accounts.cnProviders.apiProtocol.${editApiProtocol.value}Desc`
)
...
const buildModelRestrictionMapping = () => {
  const whitelistMapping = buildModelMappingObject('whitelist', allowedModels.value, [])
  const explicitMapping = buildModelMappingObject('mapping', [], modelMappings.value)
  const combined = {
    ...(whitelistMapping || {}),
    ...(explicitMapping || {})
  }
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    if (newAccount.type === 'apikey') {
      ...
      openAIEndpointCapabilities.value = readOpenAIEndpointCapabilities(accountCredentials)
      if (!openAITextGenerationCapabilityEnabled.value) {
        openAIResponsesMode.value = 'auto'
      }
    }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // For Anthropic API Key accounts, handle passthrough mode and web search emulation in extra
