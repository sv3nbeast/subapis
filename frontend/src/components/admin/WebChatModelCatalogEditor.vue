<template>
  <section class="card p-6" aria-labelledby="web-chat-model-catalog-title">
    ...
    <div v-for="(entry,index) in entries" :key="entry.id" class="mb-4 space-y-3 rounded-xl border border-gray-200 p-4 dark:border-dark-700">
      ...
      <p v-if="mismatch(entry)" class="catalog-warn" role="alert">{{ mismatch(entry) }}</p>
    </div>
...
// Mirrors the backend allowlist (isOpenAIImageGenerationModel): a mismatch is
// rejected on save, so surface it here instead of after the user hits publish.
function generatesImages(model:string):boolean{
  const m=model.trim().toLowerCase()
  return m.startsWith('gpt-image-')||m==='grok-imagine'||m==='grok-imagine-edit'||m.startsWith('grok-imagine-image')
}
function isImage(entry:WebChatCatalogEntry):boolean{return (entry.capabilities||[]).includes('image')}
function setImage(entry:WebChatCatalogEntry,value:boolean){entry.capabilities=value?['image']:['chat']}
function mismatch(entry:WebChatCatalogEntry):string{
  const model=entry.model.trim()
  if(!model)return ''
  const image=isImage(entry)
  if(image===generatesImages(model))return ''
  return image?text.value.notImageModel:text.value.needsImageFlag
}
...
<style scoped>.catalog-field{display:flex;flex-direction:column;gap:.3rem;font-size:.8rem;color:#64748b}.catalog-warn{margin-top:.5rem;font-size:.8rem;color:#b45309;background:#fffbeb;border:1px solid #fcd34d;border-radius:.5rem;padding:.5rem .625rem}</style>
