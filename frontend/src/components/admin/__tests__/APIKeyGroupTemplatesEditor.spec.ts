...
describe('APIKeyGroupTemplatesEditor', () => {
  ...
  it('creates an AWS-compatible Anthropic provider for an Anthropic group', async () => {
    ...
    expect(content.provider.anthropic.models['claude-fable-5-1'].limit).toEqual({ context: 1048576, output: 32000 })
    expect(content.provider.anthropic.models['claude-fable-5-1'].options.thinking).toEqual({ type: 'adaptive' })
    expect(content.provider.anthropic.models['claude-fable-5-1'].options.thinking).not.toHaveProperty('budgetTokens')
