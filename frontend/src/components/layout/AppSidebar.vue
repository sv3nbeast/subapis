...
const flagWebChat = makeSidebarFlag(FeatureFlags.webChat)
...
//
// 条目顺序：网页对话 → 密钥 → 用量 → 可用分组 → 模型状态 → 订阅/支付 → 兑换/资料。
// 可用分组紧挨模型状态之上，让用户"先看自己能用什么、再看对应状态"。
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  const items: NavItem[] = []
  if (withDashboard) {
    items.push({ path: '/dashboard', label: t('nav.dashboard'), icon: DashboardIcon })
  }
  items.push({ path: '/chat', label: t('nav.webChat'), icon: ChatIcon, featureFlag: flagWebChat })
  items.push(
