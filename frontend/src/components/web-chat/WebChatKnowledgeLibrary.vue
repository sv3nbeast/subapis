<template>
  <BaseDialog :show="show" :title="`${project?.name || ''} · ${t('webChat.knowledgeLibrary')}`" width="wide" @close="emit('close')">
    <div class="space-y-4" @dragover.prevent @drop.prevent="dropFiles">
      ...
      <div v-for="upload in uploads" :key="upload.name" class="upload-row"><span class="truncate">{{ upload.name }}</span><progress :value="upload.progress" max="100"/><small>{{ upload.error || `${upload.progress}%` }}</small></div>
...
const props=defineProps<{show:boolean;project:WebChatProject|null;limits:WebChatDocumentLimits}>();const emit=defineEmits<{close:[]}>();const{t}=useI18n();const documents=ref<WebChatDocument[]>([]),filter=ref('all'),uploads=ref<{name:string;progress:number;error?:string}[]>([]);let timer:ReturnType<typeof setInterval>|null=null
...
function pickFiles(e:Event){void upload(Array.from((e.target as HTMLInputElement).files||[]));(e.target as HTMLInputElement).value=''}function dropFiles(e:DragEvent){void upload(Array.from(e.dataTransfer?.files||[]))}async function upload(files:File[]){if(!props.project)return;for(const file of files){const state={name:file.name,progress:0,error:''};uploads.value.push(state);try{await webChatAPI.uploadProjectDocument(props.project.id,file,p=>state.progress=p);state.progress=100;await load()}catch(e){state.error=extractApiErrorMessage(e)}}}
