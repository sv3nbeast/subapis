  ...
  it('contains a non-empty message for every locale leaf', () => {
    for (const [locale, messages] of Object.entries({ en, zh })) {
      const emptyKeys = flattenLeafKeys(messages).filter((key) => {
        ...
        // docsGuide.*.items are string lists consumed through tm(); every entry must still be a non-empty string.
        if (Array.isArray(current)) {
          return current.length === 0 || current.some((item) => typeof item !== 'string' || item.trim() === '')
        }
