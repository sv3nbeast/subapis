...
describe('locale module integration', () => {
  it('exposes newly split Chinese modules at their runtime paths', () => {
    ...
    expect(zh.admin.groups.platforms.zhipu).toBe('智谱 GLM')
    expect(en.admin.groups.platforms.zhipu).toBe('Zhipu GLM')
    expect(zh.admin.groups.platforms.kimi).toBe('Kimi')
    expect(zh.admin.groups.platforms.deepseek).toBe('DeepSeek')
