    ...
    accounts: {
      ...
      anthropic: {
        ...
        stableIdentity: {
          ...
          description:
            'Pins this OAuth/SetupToken account to one device identity and upstream request path. Select existing Anthropic groups—no dedicated group is created. Only native Claude Code traffic from selected API keys is pinned to this account.',
          statusActive: 'Active',
          statusPaused: 'Paused',
          ...
          keys: 'Route keys',
          existingGroups: 'Select existing Anthropic groups',
          existingGroupsHint: 'The account is added to these groups. Other memberships and other accounts in each group are unchanged.',
          noGroups: 'No active existing Anthropic groups are available',
          routeKeys: 'Select API keys for the stable path',
          routeKeysHint: 'All active keys in the selected groups are selected by default. Each user keeps an independent session.',
          deselectAll: 'Deselect all',
          selectGroupFirst: 'Select at least one existing group first',
          noKeys: 'No active API keys are available in the selected groups',
          keysLoadFailed: 'Failed to load group API keys',
          scopeHint:
            'Enable, pause, and disable take effect immediately; save other edits in this dialog first. Only reviewed Claude Code versions use the stable path. Ordinary clients, unselected keys, and non-Claude Code traffic keep the existing scheduler. An unreviewed Claude Code upgrade is rejected locally instead of silently falling back to another account.',
          passthroughConflict: 'Stable identity and OAuth native strict passthrough cannot be enabled together. Turn off the switch above and save the account first.',
          enable: 'Enable stable mode',
          saveAndResume: 'Save and resume',
          apply: 'Apply groups and keys',
          pause: 'Pause',
          resume: 'Resume',
          disable: 'Disable stable mode',
          disableConfirm: 'Restore the original scheduler and groups?',
          loadFailed: 'Failed to load stable identity status',
          configureFailed: 'Failed to configure stable identity',
          pauseFailed: 'Failed to pause stable identity',
          resumeFailed: 'Failed to resume stable identity',
