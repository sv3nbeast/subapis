export default {
    accounts: {
      ...
      stableSchedulableActive: '稳定调度中',
      stableSchedulablePaused: '稳定调度已暂停',
      stableSchedulableBlocked: '稳定调度已阻断',
      stableSchedulableHint: '该账号由稳定身份专用调度器管理；请在编辑账号中使用稳定模式开关控制调度。',
      ...
      cnProviders: {
        accountMode: {
          title: '账号类型',
          payg: '按量付费',
          paygDesc: '消耗账户余额，按 Token 计费。',
          coding: 'Coding Plan',
          codingDesc: '订阅制编程套餐，按滚动用量窗口限流。',
        },
        apiProtocol: {
          title: 'API 协议',
          adaptive: '自适应',
          adaptiveDesc: '优先使用供应商原生端点，仅在缺少对应端点时转换。',
          endpoints: '协议端点',
          responsesFallbackDesc: '该供应商没有原生 Responses 端点，请求将转换为 Chat Completions。',
          chatCompletions: 'Chat Completions',
          chatCompletionsDesc: '使用 OpenAI 兼容的 Chat Completions 端点。',
          anthropic: 'Anthropic',
          anthropicDesc: '使用供应商原生 Anthropic 端点，适配 Claude Code。',
          responses: 'Responses',
          responsesDesc: '使用供应商原生 Responses 端点，适配 Codex。',
        },
        zhipuTeam: {
          title: '团队版组织 / 项目 ID',
          ...
          hint: '仅团队版 GLM Coding Plan 需要填写，个人版留空即可。',
          help: {
            title: '如何获取组织 / 项目 ID',
            step1: '进入智谱控制台的「Coding Plan → 团队版 → 我的套餐」。',
            step2: '打开浏览器开发者工具，切换到 Network 后刷新页面。',
            step3: '筛选 /api/biz/v1/organization 并打开命中的请求。',
            step4: '从请求 URL 或请求头复制 org-… 与 proj_… 的值。',
            example: '示例：…/organization/org-…/projects/proj_…/api_keys',
          },
        },
        balance: '余额 --',
        window5h: '5h',
        ...
        windowMonthly: '月',
        probe: '查询',
        probeTooltip: '查询供应商滚动窗口额度',
        balanceProbeTooltip: '查询供应商账户余额',
        balanceLow: '余额不足',
      ...
      grok: {
        ...
        imagePromptHint: '使用所选 Grok 生图模型发送图片生成探测请求。',
        // 账号连通性测试弹窗
        testMode: '测试模式',
        testModeHint:
      ...
      anthropic: {
        ...
        stableIdentity: {
          ...
          loadFailed: '读取稳定身份状态失败',
          configureFailed: '稳定身份配置失败',
          disableFailed: '关闭稳定身份失败',
        },
      },
      ...
      cursor: {
        ...
        mixedSchedulingHint: '启用后可参与 Claude 分组的调度，支持工具调用（Claude Code 可用）。带图片的请求会自动让给同组其它账号，不会失败。',
