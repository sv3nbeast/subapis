    ...
    accounts: {
      ...
      anthropic: {
        ...
        stableIdentity: {
          title: '稳定身份模式',
          description:
            '固定该 OAuth/SetupToken 账号的设备身份与上游请求链路。直接选择现有 Anthropic 分组，不会创建专用分组；仅所选 API Key 的原生 Claude Code 请求会固定使用此账号。',
          statusActive: '运行中',
          statusPaused: '已暂停',
          statusBlocked: '已阻断',
          statusOff: '未启用',
          ...
          groups: '现有分组',
          keys: '路由 Key',
          existingGroups: '选择现有 Anthropic 分组',
          existingGroupsHint: '账号会加入所选分组，已有的其他分组和分组内其他账号不会被改动。',
          noGroups: '没有可用的现有 Anthropic 分组',
          routeKeys: '选择进入稳定路径的 API Key',
          routeKeysHint: '默认选择所选分组内全部有效 Key；不同用户仍保留各自独立会话。',
          deselectAll: '取消全选',
          selectGroupFirst: '先选择至少一个现有分组',
          noKeys: '所选分组没有有效 API Key',
          keysLoadFailed: '读取分组 API Key 失败',
          scopeHint:
            '这里的启用、暂停和关闭会立即生效，请先保存本窗口中的其他修改。只有已审核版本的 Claude Code 请求进入稳定路径；同一分组中的普通客户端、未选中的 Key 和非 Claude Code 请求继续使用原调度。Claude Code 升级为尚未审核的版本时会本地拒绝，不会悄悄回落到其他账号。',
          passthroughConflict: '稳定身份模式与“OAuth 原生严格透传”不能同时开启。请先关闭上方开关并保存账号。',
          enable: '启用稳定模式',
          saveAndResume: '保存并恢复',
          apply: '应用分组与 Key',
          pause: '暂停',
          resume: '恢复',
          disable: '关闭稳定模式',
          disableConfirm: '恢复原调度和原分组？',
          loadFailed: '读取稳定身份状态失败',
          configureFailed: '稳定身份配置失败',
          pauseFailed: '暂停稳定身份失败',
          resumeFailed: '恢复稳定身份失败',
