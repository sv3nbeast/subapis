...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    path: '/docs',
    name: 'DocsGuide',
    component: () => import('@/views/public/DocsGuideView.vue'),
    meta: {
      requiresAuth: false,
      title: 'Docs',
      titleKey: 'home.guide'
  ...
  },
  {
    ...
    meta: {
      ...
      requiresAdmin: false,
      title: 'Batch Image',
      titleKey: 'batchImage.title',
      descriptionKey: 'batchImage.description'
    }
  },
  {
    path: '/usage',
  ...
  },
  {
    ...
    component: () => import('@/views/public/ChannelStatusPublicView.vue'),
    meta: {
      requiresAuth: false,
...
const BACKEND_MODE_ALLOWED_PATHS = ['/login', '/key-usage', '/docs', '/setup', '/payment/result', '/payment/airwallex', '/legal', '/monitor', '/status']
...
router.beforeEach(async (to, _from, next) => {
  ...
  if (to.name === 'CustomPage') {
    ...
    if (menuItem?.label) {
      const siteName = appStore.siteName || 'subapis'
  ...
  if (
    (to.meta.requiresPayment || to.meta.requiresRiskControl || to.meta.requiresWebChat) &&
  ...
  if (
    to.meta.requiresWebChat &&
    appStore.publicSettingsLoaded &&
    !isFeatureFlagEnabled(FeatureFlags.webChat)
  ) {
