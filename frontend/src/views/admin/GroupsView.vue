  ...
  <AppLayout>
    ...
    >
      ...
      >
            ...
            >
              ...
              <div class="flex items-center gap-1.5">
                ...
                <button
                  type="button"
                  class="rounded px-2 py-1 font-medium text-gray-600 transition-colors hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
                  @click="loadModelAllowlistCandidates('create', 0, createForm.platform)"
                >
                  {{ t("common.refresh") }}
                </button>
              </div>
        ...
        <div
          v-if="supportsNonStreamMessages(createForm.platform)"
          class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4"
        >
          <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
            {{ t("admin.groups.nonStreamMessages.title") }}
          </h4>
          <div class="flex items-center justify-between">
            <label class="text-sm text-gray-600 dark:text-gray-400">{{
              t("admin.groups.nonStreamMessages.allow")
            }}</label>
            <button
              type="button"
              @click="
                createForm.allow_non_stream_messages =
                  !createForm.allow_non_stream_messages
              "
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {{ t("admin.groups.nonStreamMessages.hint") }}
          </p>
        </div>
    ...
    >
      ...
      >
            ...
            >
              ...
              <div class="flex items-center gap-1.5">
                ...
                <button
                  type="button"
                  class="rounded px-2 py-1 font-medium text-gray-600 transition-colors hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
                  @click="loadModelAllowlistCandidates('edit', editingGroup?.id || 0, editForm.platform)"
                >
                  {{ t("common.refresh") }}
                </button>
              </div>
        ...
        <div
          v-if="supportsNonStreamMessages(editForm.platform)"
          class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4"
        >
          <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
            {{ t("admin.groups.nonStreamMessages.title") }}
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              type="button"
              @click="
                editForm.allow_non_stream_messages =
                  !editForm.allow_non_stream_messages
              "
              class="relative inline-flex h-6 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none"
              :class="
...
const groupPricingFromAPI = (
  ...
  (pricing || []).map((entry) => ({
    ...
    cache_write_5m_price: perTokenToMTok(entry.cache_write_5m_price),
...
const supportsNonStreamMessages = (platform: string) =>
  ["anthropic", "gemini", "antigravity", "kiro", "droid"].includes(
    String(platform || ""),
  );
...
const handleCreateGroup = async () => {
  ...
  try {
    ...
    const requestData = {
      ...createGroupForm,
      video_model_prices: serializeVideoModelPrices(createForm.video_model_prices),
      ...
      allow_non_stream_messages: createForm.allow_non_stream_messages,
...
const handleEdit = async (group: AdminGroup) => {
  ...
  editForm.video_model_prices = group.video_model_prices
    ? { ...createVideoModelPricesForm(), ...group.video_model_prices }
    : createVideoModelPricesForm();
  ...
  editForm.allow_non_stream_messages =
    group.allow_non_stream_messages ?? false;
...
const closeEditModal = () => {
  ...
  editForm.allow_non_stream_messages = false;
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      allow_non_stream_messages: editForm.allow_non_stream_messages,
      reasoning_effort_mappings: reasoningEffortMappingsToAPI(
        editForm.reasoning_effort_mappings,
      ),
      messages_dispatch_model_config:
