                  ...
                  >
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          :value="formatSubscribedAt(provider.subscribed_at ?? null)"
                          type="date"
                          class="input text-sm"
                          @input="
              ...
              <template v-if="form.payment_enabled">
                ...
                <div class="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    >
                      {{
                        (form.payment_product_name_prefix || "subapis") +
                        " 100 " +
...
const defaultLoginAgreementDate = "2026-05-14";
...
您应对账号、API Key、访问令牌、代理凭据、OAuth 授权、订阅分组、组织项目和其他凭据下发生的所有活动负责。您应妥善保管凭据。如发现凭据泄露、异常调用、拒绝服务攻击、滥用或其他安全事件，应立即通知管理员并采取必要措施。

## 3. 客户内容与输出

您保留对输入内容的权利，并在适用法律允许范围内拥有输出内容。AI 输出可能不准确、不完整、过时、带有偏见或不适合特定用途；在依赖、发布或用于重要场景前，您应自行审查和验证。

## 4. 合规与限制

您及最终用户必须遵守适用法律法规、出口管制、制裁规定、第三方服务条款、模型使用政策和本平台规则。不得绕过限流、鉴权、计费、风控、地区限制或安全机制，不得逆向工程、抓取、批量提取或使用服务构建竞争性平台、模型或评测体系，除非获得明确书面授权。

## 5. 费用、额度与变更

平台可根据套餐、分组、倍率、模型价格、请求类型、缓存、图片、工具调用或其他配置计费。第三方模型价格、汇率、服务层级和计费规则可能变化，平台可据此调整计费配置并在合理范围内通知。

## 6. 暂停与终止

如您违反条款、使用政策、第三方规则或法律要求，或您的使用可能造成安全、合规、运营、账务或上游账号风险，平台可限制、暂停、终止服务或拒绝相关请求。

## 7. 免责声明

服务按“现状”和“可用”提供。除法律强制要求外，平台不对第三方服务可用性、模型输出准确性、特定用途适用性、不中断或无错误作出保证。

---

**Effective Date: 2026-05-14**

Welcome to subapis. These Terms govern your or your organization's access to and use of API aggregation, key management, subscription conversion, billing, monitoring, tools, documentation, and related services. By using the Services, you confirm that you have read, understood, and agreed to these Terms, the Usage Policy, the Supported Regions Policy, and the Service-Specific Terms.
...
You are responsible for all activity under your account, API keys, access tokens, proxy credentials, OAuth grants, subscription groups, organizations, projects, and other credentials. You must protect credentials from unauthorized access and notify the administrator promptly if you suspect compromise or abuse.

## 3. Customer Content and Outputs

You retain rights in inputs and, to the extent permitted by law, own outputs. AI outputs may be inaccurate, incomplete, outdated, biased, or unsuitable for a particular use case; you are responsible for reviewing and verifying outputs before relying on or sharing them.

## 4. Compliance and Restrictions

You and your end users must comply with applicable laws, export controls, sanctions rules, third-party terms, model usage policies, and platform rules. You may not bypass rate limits, authentication, billing, risk controls, regional restrictions, or security measures; reverse engineer, scrape, bulk extract, or use the Services to build a competing platform, model, or benchmark system without express written authorization.

## 5. Fees, Quotas, and Changes

The platform may bill based on plans, groups, multipliers, model prices, request types, cache usage, image generation, tool calls, or other configuration. Third-party prices, exchange rates, service tiers, and billing rules may change, and platform billing configuration may be adjusted accordingly.

## 6. Suspension and Termination

The platform may restrict, suspend, terminate, or reject requests if you violate these Terms, the Usage Policy, third-party rules, or legal requirements, or if your use creates security, compliance, operational, billing, or upstream-account risk.

## 7. Disclaimer

The Services are provided “as is” and “as available.” Except where prohibited by law, the platform makes no warranties regarding third-party availability, output accuracy, fitness for a particular purpose, uninterrupted operation, or error-free service.`;
...
当请求被转发到第三方模型、云平台或兼容接口时，您还必须遵守该上游的服务条款、使用政策、支持地区政策、数据处理条款和模型特定限制。
...
When requests are forwarded to third-party models, cloud platforms, or compatible APIs, you must also comply with the upstream provider's terms of service, usage policies, supported-region policies, data-processing terms, and model-specific restrictions.`;
...
您不得从适用法律、制裁、出口管制、上游服务条款或平台规则禁止的地区访问或使用服务，也不得向受限制人员、实体、国家、行业或用途提供访问。
...
Even if the platform itself is reachable, certain third-party models, OAuth services, cloud platforms, payment methods, proxy networks, CAPTCHA, SMS, or subscription services may be available only in specific countries or regions. Upstream services may enforce restrictions based on IP address, account registration location, payment method, project, organization, device, identity verification, or risk signals.

## 2. Prohibited or Restricted Regions

You may not access or use the Services from regions prohibited by applicable law, sanctions, export controls, upstream terms, or platform rules, and you may not provide access to restricted persons, entities, countries, industries, or use cases.
...
平台可为 OpenAI、Anthropic、Gemini、Antigravity、Claude Code、Codex 或其他协议提供模型映射、请求转换和响应桥接。映射不代表上游存在同名模型，也不保证不同模型在能力、价格、上下文、工具、图片、思考、缓存或输出格式上完全一致。

## 3. OAuth、订阅和第三方账号

使用 OAuth、订阅账号、API Key、服务账号或上游凭据时，您必须拥有合法授权。第三方账号可能触发验证码、短信验证、地区验证、隐私设置、风控、额度限制、封禁、降级或服务条款提示。
...
The platform may provide model mapping, request transformation, and response bridging for OpenAI, Anthropic, Gemini, Antigravity, Claude Code, Codex, or other protocols. A mapping does not mean that an upstream model with the same name exists, and it does not guarantee identical capability, price, context length, tools, image support, reasoning behavior, cache behavior, or output format.

## 3. OAuth, Subscriptions, and Third-Party Accounts

When using OAuth, subscription accounts, API keys, service accounts, or upstream credentials, you must have lawful authorization. Third-party accounts may trigger CAPTCHA, SMS verification, regional verification, privacy settings, risk controls, quota limits, bans, downgrades, or terms-of-service notices.
...
const form = reactive<SettingsForm>({
  ...
  login_agreement_updated_at: defaultLoginAgreementDate,
...
async function loadSettings() {
  ...
  try {
    ...
    form.login_agreement_updated_at =
      settings.login_agreement_updated_at || defaultLoginAgreementDate;
