              ...
              >
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    <Select
                      ...
                      @click.stop
                    />
                    <span v-if="!expandedProviders[pIdx] && (provider.quota_limit ?? 0) > 0" class="text-xs text-gray-400">
                      {{ provider.quota_used ?? 0 }} / {{ provider.quota_limit ?? 0 }}
                    </span>
                ...
                >
                  <div>
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <button
                        ...
                        @click="copyWebSearchApiKey(pIdx)"
                      >
                  ...
                  <div v-if="(provider.quota_limit ?? 0) > 0" class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-gray-500">{{ provider.quota_used ?? 0 }} / {{ provider.quota_limit ?? 0 }}</span>
                  </div>
                  ...
                  <div class="border-t border-gray-100 pt-3 dark:border-dark-700">
                    ...
                    <div v-if="wsTestResult" class="mt-2 rounded-lg bg-gray-50 p-3 text-xs dark:bg-dark-700">
                      ...
                      <div v-for="(result, idx) in wsTestResult.results.slice(0, 3)" :key="idx" class="mt-1">
                        <a :href="result.url" target="_blank" class="font-medium text-blue-600 hover:underline dark:text-blue-400">
                          {{ result.title }}
                        </a>
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              ...
              <a
                :href="locale === 'zh' ? 'https://github.com/Wei-Shaw/sub2api/blob/main/docs/PAYMENT_CN.md' : 'https://github.com/Wei-Shaw/sub2api/blob/main/docs/PAYMENT.md'"
                target="_blank"
                rel="noopener noreferrer"
              ...
              >
                <span class="text-sm font-medium">{{ t('admin.settings.payment.configGuide') }}</span>
                <svg class="mb-0.5 ml-1 h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
              </a>
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            ...
            <template v-if="form.payment_enabled">
              ...
              <div>
                ...
                <p class="mt-2 text-xs text-gray-400 dark:text-gray-500">
                  ...
                  <a
                    :href="locale === 'zh' ? 'https://github.com/Wei-Shaw/sub2api/blob/main/docs/PAYMENT_CN.md#%E6%94%AF%E6%8C%81%E7%9A%84%E6%94%AF%E4%BB%98%E6%96%B9%E5%BC%8F' : 'https://github.com/Wei-Shaw/sub2api/blob/main/docs/PAYMENT.md#supported-payment-methods'"
                    target="_blank"
                    rel="noopener noreferrer"
...
async function copyWebSearchApiKey(idx: number) {
...
async function loadWebSearchConfig() {
  try {
    const [config, proxyResponse] = await Promise.all([
      adminAPI.settings.getWebSearchEmulationConfig().catch((error: any) => {
...
function removeRegistrationEmailSuffixBlacklistTag(suffix: string) {
  registrationEmailSuffixBlacklistTags.value = registrationEmailSuffixBlacklistTags.value.filter(
    (item) => item !== suffix
  )
}
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      registration_email_suffix_blacklist: registrationEmailSuffixBlacklistTags.value.map(
        (suffix) => `@${suffix}`
      ),
