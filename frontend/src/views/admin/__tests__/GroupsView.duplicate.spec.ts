...
describe('GroupsView duplicate action', () => {
  ...
  it.each(['kimi', 'zhipu', 'deepseek'])('creates a %s group through the rendered platform selector', async (platform) => {
    ...
    getModelAllowlistCandidates.mockResolvedValue(['public-model'])
    const wrapper = mountView()
    await flushPromises()
    ...
    await flushPromises()
    // 官方白名单 UI 仅在开关开启后渲染候选项，这里只断言候选项按平台拉取。
    expect(getModelAllowlistCandidates).toHaveBeenCalledWith(0, platform)
