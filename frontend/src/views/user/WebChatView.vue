  ...
  <AppLayout>
    <div ref="webChatShellRef" class="web-chat-shell" :style="webChatShellStyle">
      ...
      <main class="chat-panel">
        ...
        <WebChatComposer v-model="draft" :disabled="!canCompose" :can-send="canSend&&Boolean(draft.trim())" :sending="sending" :files-enabled="options.files_enabled" :templates-enabled="options.templates_enabled" :template-name="activeTemplateName" :documents="pendingDocuments" :attachment-state="attachmentState" @submit="send" @stop="stopGeneration" @open-template="templateDialogOpen=true" @files="uploadTemporaryDocuments" @remove-document="removePendingDocument"/>
      </main>
...
const {pendingDocuments,attachmentState,uploadTemporaryDocuments,removePendingDocument,clearPendingDocuments}=useWebChatDocuments(async()=>activeSession.value||await createSessionForCurrentSelection(),key=>t(key))
...
function startDraftSession(){activeSessionId.value=null;messages.value=[];streamingText.value='';streamingSources.value=[];clearPendingDocuments();sessionsOpen.value=false;const p=selectedProject.value;if(p?.default_group_id)selectedGroupId.value=p.default_group_id;if(p?.default_model)selectedModel.value=p.default_model;activeTemplateId.value=p?.default_template_id??null}
