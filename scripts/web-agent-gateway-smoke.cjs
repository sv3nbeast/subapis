...
;(async () => {
  ...
  const library = await api('/web-chat/artifacts')
  assert(library.items.length >= results.length, 'Global artifact library must include session results')
  const lookedUp = await api(`/web-chat/sessions/${session.id}`)
  assert.equal(lookedUp.id, session.id)
  const foreign = await fetch(base + `/web-chat/sessions/${session.id}`, { headers: { Authorization: 'Bearer invalid-other-user' } })
  assert([401, 403, 404].includes(foreign.status), 'Unauthenticated/foreign session access must be rejected')
  ...
  console.log(JSON.stringify({ session_id: session.id, tasks: results, library_count: library.items.length, model_requests: after.generations - before.generations, chat_done: true, storage }, null, 2))
